...
const ACCENT_COLORS = [
  "var(--green)", "var(--cyan)", "var(--purple)",
  "#f59e0b", "#f472b6", "#34d399", "#60a5fa",
  "#a78bfa", "#fb7185", "#fbbf24",
];

const TYPE_LABELS: Record<string, string> = {
  EQUITY: "Stock", ETF: "ETF", CRYPTOCURRENCY: "Crypto",
  MUTUALFUND: "Fund", INDEX: "Index", FUTURE: "Future",
};
...
export default function PortfolioBuilder({ assets, setAssets }: Props) {
  ...
  const selectTicker = (i: number, result: SearchResult) => {
    ...
    setSearchResults(prev => ({ ...prev, [i]: [] }));
    const next = [...assets];
    next[i] = { ...next[i], ticker: result.ticker };
    setAssets(next);
    setActiveIndex(null);
  ...
  return (
    <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-green)", borderRadius: 12, padding: "20px", position: "relative", overflow: "visible" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          <button
            ...
            title="Import from brokerage screenshot"
            style={{ background: "rgba(0,212,255,0.08)", border: "1px solid rgba(0,212,255,0.2)", borderRadius: 6, padding: "3px 8px", fontSize: 9, color: "var(--cyan)", cursor: "pointer", letterSpacing: 1, fontFamily: "var(--font-display)", display: "flex", alignItems: "center", gap: 4, transition: "all 0.2s" }}
          ...
          <span
            ...
            onClick={!isBalanced ? equalizeWeights : undefined}
            title={!isBalanced ? "Click to equalize weights" : ""}
          >
      ...
      {importError && (
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ fontSize: 10, color: "var(--red)", marginBottom: 10, textAlign: "center" }}>{importError}</motion.p>
      )}
      ...
      <AnimatePresence>
        {assets.map((a, i) => {
          ...
          const inputVal = query[i] ?? a.ticker;
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ position: "relative", zIndex: activeIndex === i ? 60 : 1, flex: 1 }}>
                  <input
                    ...
                    onBlur={() => {
                      blurTimers.current[i] = setTimeout(() => {
                    ...
                    style={{
                      ...
                      border: `1px solid ${activeIndex === i ? color : "var(--border-dim)"}`,
                      borderRadius: 7, color,
                      fontFamily: activeIndex === i && inputVal.length > 2 ? "var(--font-body)" : "var(--font-display)",
                      fontSize: 11, fontWeight: 700, letterSpacing: activeIndex === i && inputVal.length > 2 ? 0 : 1,
                      ...
                      >
                        {results.map((r, idx) => (
                          <div key={idx}
                            ...
                            style={{ padding: "9px 12px", display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", borderBottom: idx < results.length - 1 ? "1px solid rgba(255,255,255,0.04)" : "none", transition: "background 0.1s" }}
                            onMouseEnter={e => (e.currentTarget.style.background = "rgba(0,255,160,0.06)")}
                            onMouseLeave={e => (e.currentTarget.style.background = "transparent")}
                          >
                            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                              <span style={{ fontFamily: "var(--font-display)", fontSize: 12, color: "var(--green)", letterSpacing: 1 }}>{r.ticker}</span>
                              <span style={{ fontSize: 10, color: "var(--text-muted)", maxWidth: 160, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.name}</span>
                            </div>
                            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 2 }}>
                              <span style={{ fontSize: 8, color: "var(--text-muted)", letterSpacing: 1 }}>{r.exchange}</span>
                              <span style={{ fontSize: 8, background: "rgba(0,212,255,0.1)", color: "var(--cyan)", padding: "1px 5px", borderRadius: 3, letterSpacing: 1 }}>{TYPE_LABELS[r.type] || r.type}</span>
                            </div>
                          </div>
                        ))}
                      </motion.div>
                ...
                <button onClick={() => remove(i)}
                  ...
                  onMouseLeave={e => (e.currentTarget.style.color = "rgba(255,77,109,0.35)")}
                >✕</button>
              </div>

              {/* Name label */}
              {name && (
                <div style={{ paddingLeft: 13, marginTop: 3, fontSize: 9, color: "var(--text-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                  {name}
                </div>
              )}
      ...
      <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
        ...
        {!isBalanced && assets.length > 0 && (
          <button onClick={equalizeWeights}
            style={{ padding: "8px 12px", background: "rgba(0,212,255,0.06)", border: "1px solid rgba(0,212,255,0.2)", borderRadius: 8, color: "var(--cyan)", fontSize: 9, letterSpacing: 1, fontFamily: "var(--font-display)", cursor: "pointer", transition: "all 0.2s" }}

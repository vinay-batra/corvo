...
function LiveClock() {
  ...
  return <span style={{ fontFamily: "var(--font-display)", fontSize: 11, color: "var(--text-muted)", letterSpacing: 2 }}>{time}</span>;
}
...
function GlassCard({ children, accent, title, style = {} }: any) {
  return (
    ...
    >
      ...
      {title && <p style={{ fontSize: 9, letterSpacing: 3, color: "var(--text-muted)", textTransform: "uppercase", marginBottom: 16 }}>{title}</p>}
      {children}
    </motion.div>
  );
}
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", height: 360, gap: 16 }}>
      {loading ? (
        <>
          ...
          <p style={{ fontSize: 12, letterSpacing: 2, color: "var(--text-muted)", textTransform: "uppercase" }}>Crunching the numbers...</p>
        </>
      ) : (
        <>
          <div style={{ fontFamily: "var(--font-display)", fontSize: 48, color: "var(--border-mid)" }}>◈</div>
          <p style={{ fontSize: 12, letterSpacing: 2, color: "var(--text-muted)", textTransform: "uppercase" }}>Add assets to begin analysis</p>
        </>
      )}
    </div>
  );
      ...
      >
        <div style={{ padding: "24px 24px 16px", borderBottom: "1px solid var(--border-dim)" }}>
          <a href="/" style={{ textDecoration: "none" }}>
            <div style={{ fontFamily: "var(--font-display)", fontSize: 20, fontWeight: 900, letterSpacing: 5, color: "var(--green)", textShadow: "0 0 24px rgba(0,255,160,0.4)" }}>
              ALPHA<span style={{ color: "rgba(255,255,255,0.3)" }}>i</span>
            </div>
          </a>
          <div style={{ fontSize: 9, letterSpacing: 3, color: "var(--text-muted)", marginTop: 3, textTransform: "uppercase" }}>Portfolio Intelligence</div>
        ...
        <div style={{ padding: "12px 16px 0" }}>
          <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-dim)", borderRadius: 12, padding: "14px 16px" }}>
            <p style={{ fontSize: 9, letterSpacing: 3, color: "var(--text-muted)", textTransform: "uppercase", marginBottom: 10 }}>Period</p>
        ...
        <div style={{ padding: "8px 16px 0" }}>
          <div style={{ background: "var(--bg-card)", border: "1px solid var(--border-dim)", borderRadius: 12, padding: "14px 16px" }}>
            ...
            >
              <p style={{ fontSize: 9, letterSpacing: 3, color: "var(--text-muted)", textTransform: "uppercase" }}>Benchmark</p>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                ...
                <span style={{ color: "var(--text-muted)", fontSize: 10 }}>{showBenchmarks ? "▲" : "▼"}</span>
              </div>
            </button>
            <AnimatePresence>
              {showBenchmarks && (
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12 }}>
                      <GlassCard accent="var(--red)" title="Max Drawdown">
                        ...
                        <p style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 8 }}>Worst peak-to-trough decline in the selected period.</p>
                      </GlassCard>
                      <GlassCard accent="var(--cyan)" title="Annualized Volatility">
                        ...
                        <p style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 8 }}>Standard deviation of daily returns × √252.</p>
                      </GlassCard>
                      <GlassCard accent="var(--green)" title="Sharpe Ratio">
                        ...
                        <p style={{ fontSize: 12, color: "var(--text-secondary)", marginTop: 8 }}>Risk-adjusted return above 4% risk-free rate.</p>
                      </GlassCard>
                    </div>
                    ...
                    <GlassCard accent="var(--purple)" title="About This Simulation">
                      ...
                      <div style={{ marginTop: 20, display: "flex", flexDirection: "column", gap: 12 }}>
                        {[
                          ...
                          { label: "Inputs", value: "Historical μ, σ", color: "var(--text-secondary)" },
                        ].map(r => (
                          <div key={r.label} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid var(--border-dim)", paddingBottom: 10 }}>
                            <span style={{ fontSize: 11, color: "var(--text-muted)", letterSpacing: 1 }}>{r.label}</span>
                            <span style={{ fontSize: 11, fontFamily: "var(--font-display)", color: r.color, letterSpacing: 1 }}>{r.value}</span>
                          </div>
                        ))}
                      </div>
                      <p style={{ marginTop: 16, fontSize: 10, color: "var(--text-muted)", lineHeight: 1.6 }}>

    ...
    >
      <AnimatePresence mode="wait">
        <motion.div
          ...
          style={{ background: "var(--card-bg)", border: `1px solid ${current.color}40`, borderRadius: 18, padding: "32px 36px", maxWidth: 460, width: "100%", position: "relative" }}
        >
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 24 }}>
            {STEPS.map((_, i) => (
              <div key={i} style={{ width: i === step ? 20 : 6, height: 6, borderRadius: 3, background: i <= step ? current.color : "var(--border)", transition: "all 0.3s", cursor: "pointer" }} onClick={() => setStep(i)} />
            ))}
          </div>

          <p style={{ fontSize: 32, marginBottom: 12 }}>{current.icon}</p>
          <h3 style={{ fontSize: 20, fontWeight: 700, color: "var(--text)", marginBottom: 10, lineHeight: 1.3 }}>{current.title}</h3>
          <p style={{ fontSize: 14, color: "var(--text2)", lineHeight: 1.7, marginBottom: 28 }}>{current.body}</p>

          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 11, color: "var(--text3)", letterSpacing: 1 }}>{step + 1} of {STEPS.length}</span>
            <div style={{ display: "flex", gap: 10 }}>
              {step < STEPS.length - 1 ? (
                <>
                  <button onClick={onComplete} style={{ fontSize: 11, color: "var(--text3)", background: "none", border: "none", cursor: "pointer", letterSpacing: 1 }}>SKIP TOUR</button>
                  <motion.button
                    onClick={() => setStep(s => s + 1)}
                    whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                    style={{ padding: "10px 22px", background: current.color, border: "none", borderRadius: 8, color: "var(--bg)", fontSize: 11, fontWeight: 700, letterSpacing: 2, cursor: "pointer", fontFamily: "var(--font-display)" }}
                  >NEXT →</motion.button>
                </>
              ) : (
                <motion.button
                  onClick={onComplete}
                  whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }}
                  style={{ padding: "10px 28px", background: current.color, border: "none", borderRadius: 8, color: "var(--bg)", fontSize: 11, fontWeight: 700, letterSpacing: 2, cursor: "pointer", fontFamily: "var(--font-display)" }}

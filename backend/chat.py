...
def build_system_prompt(portfolio_context: dict) -> str:
    ...
    if goals and goals.get("age"):
        ...
        years_until_retirement = int(goals.get("retirement_age", 65)) - int(goals.get("age", 30))
        goals_section = f"""
USER PROFILE (ALWAYS personalise advice to this person):
- Age: {goals.get("age")}
...
Reference this profile in every response. e.g. "At {goals.get("age")} with {years_until_retirement} years until retirement..."
"""

    tickers = portfolio_context.get("tickers", [])
    weights = portfolio_context.get("weights", [])
    ret  = portfolio_context.get("portfolio_return", 0) or 0
    vol  = portfolio_context.get("portfolio_volatility", 0) or 0
    dd   = portfolio_context.get("max_drawdown", 0) or 0
...
- Annualised Volatility: {vol*100:.2f}%
    ...
    response = client.messages.create(
        ...
        messages=[{
            ...
            "content": [
                ...
                {
                    "type": "text",
                    "text": 'Extract all stock/ETF holdings from this brokerage screenshot. Return ONLY a JSON array like: [{"ticker": "AAPL", "weight": 0.25}]. Use percentage allocations as weights (0–1). If no percentages, distribute equally. Return only the JSON array, nothing else.'
                }
            ]
        }]
    )

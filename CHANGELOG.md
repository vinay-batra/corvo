...
Pure prompt-engineering on `backend/main.py` /chat. No code paths changed, no schema migration, no frontend touch. Three new blocks layered into the existing system prompt to make Haiku land in the Corvo voice more reliably + cap length variance across responses.
...
- **Tone calibration anchors**: explicit "too cautious" / "too aggressive" / "too formal" failure modes paired with the matching "Corvo voice" rewrite. Trains the model to recognize and avoid each by direct contrast rather than abstract rule.
- **Roast mode length spec**: 160-190 target / 200 hard ceiling. The previous prompt left roast length unbounded so roasts were running 250+ words and losing punch. Plus a full 180-word example roast as the length anchor.
...
### Added
- **Per-holding account-type tagging.** PortfolioBuilder chevron-expand now has an Account Type selector (cash + non-cash rows both) so a single saved portfolio can mix Taxable, Roth IRA, Traditional IRA, 401(k), HSA, 529, and custodial holdings. Selector renders as a native `<select>` grouped by tax category (Taxable / Retirement / Health / Education) with "Use portfolio default" as the always-present fallback option. New `accountType?: AccountTypeId` field on the `Asset` interface, new `updateAccountType` handler, new `HoldingAccountTypeSelector` helper rendered in both the cash and non-cash expand-row variants. Persisted via new `portfolios.holding_account_types text[]` column (migration `20260517000000_portfolios_holding_account_types.sql`) - parallel to `tickers` and `weights`, with empty string in the array meaning "use portfolio default". SavedPortfolios `fromDb` / `toDb` round-trip the array, so saved mixed-account portfolios survive reload.
- **Bucketed AI tax advice.** Backend `/chat` reads `portfolio_context.holding_account_types` (parallel array to tickers/weights) and, when at least one tag differs from the portfolio default, injects a `PER-HOLDING ACCOUNT BUCKETS` block listing the holdings grouped by bucket plus the per-bucket rules (no TLH inside Roth/401k/HSA/529, cap gains only apply to Taxable, etc). New `_build_account_type_text` helper at the bottom of the prompt-rules block. Falls back cleanly to v0.32 portfolio-level behavior when no tags are set.
- **Monte Carlo fan chart.** Backend `/montecarlo` now returns 250 stratified sample paths as `paths_sample` (parallel to the existing percentile bands). MonteCarloChart renders the 250 paths as semi-transparent hairlines behind the percentile fan so the user sees actual variance - including loss scenarios - instead of just an abstract median + band. Sample-paths trace ships first in the Plotly `data` array so the bands and median line sit on top visually. New "Sample paths" entry in the chart legend.
- **Loss probability surface.** Backend returns a new `loss_probability` field (the fraction of paths that end below starting value); frontend shows it inline in the "What this means for you" insight ("X% chance of positive returns (Y% chance of a loss)").
- **Chat: Corvo company / founder / how-it-works questions now answered.** v0.40 system prompt was too tight; questions like "who is the founder of Corvo" got refused as "outside what I can help with here." Added an explicit `WHAT YOU CAN HELP WITH (do NOT refuse these)` block: portfolio-related anything, Corvo-the-product anything (founder = Vinay Batra, high school sophomore, solo founder, why-built rationale baked in), general financial education, small talk. Refusals reserved for actually dangerous / illegal / out-of-scope (medical, legal contracts, college essay). When a refusal is warranted, do it in one sentence and pivot back to the portfolio - never a long disclaimer.

### Changed
- **Monte Carlo math rewritten for realism.** Dropped the artificial 8% sigma floor (was suppressing real-world variance enough that the 5th-percentile fan band never showed losses for diversified portfolios), switched from Gaussian to Student-t (df=6) innovations for empirical fat tails (so 2008 / 2020 / 2022-style years appear at realistic frequency rather than being 5-sigma "impossible" events the Gaussian model would never produce), lowered the mu cap from 12% to 10% (more honest long-horizon projection), and removed the 0% mu floor in `/portfolio/retirement-simulation` that was masking portfolios with genuinely poor historical performance behind a "we'll assume you break even" assumption. Result: a 5-year SPY simulation now shows `final_p5 ≈ -0.76%` and `loss_probability ≈ 5.3%` instead of the old all-positive bear case. ASSET_CLASS_MU expanded to cover VTSAX / VFIAX / VLCAX / VFWAX / VXUS so mutual-fund-heavy portfolios get an appropriate anchor instead of falling through to the per-stock branch.
- **Monte Carlo path count bumped 8,500 → 10,000 everywhere.** Backend `/montecarlo`, `/montecarlo/insight`, `/portfolio/retirement-simulation`. Frontend `lib/api.ts`, `MonteCarloChart`, `RetirementSimulator`, `app/page.tsx` (5 refs), `app/app/page.tsx` (2 refs), `app/demo/page.tsx`, `app/learn/page.tsx`, `app/changelog/page.tsx`, `app/faq/page.tsx`, `BlogFilteredPosts.tsx`, `monte-carlo-simulation-investing/page.tsx` (7 refs + anchor `why-10000-paths`), `bloomberg-alternative/page.tsx`, `building-corvo-what-we-learned/page.tsx`, `portfolio-diversification-guide/page.tsx`, 3 `compare/*` testimonials, plus README, CHANGELOG, CLAUDE.md. CLAUDE.md rule updated: simulations always run exactly 10,000 paths - never 5,000, 8,500, or any other number.
- **Monte Carlo table labels: "Bear Case" / "Bull Case" → "Worst 5%" / "Best 5%".** Was misleading when the underlying 5th-percentile was actually positive (which for a long-horizon diversified portfolio it often is). Worst/Best edges now flip green/red based on the actual sign of the change, not based on the label.
- **Monte Carlo VaR and Expected Shortfall cards now honest about positive worst-cases.** Used to always render as `-X%` in red even when the underlying p5 was positive (so a SPY-heavy 10y portfolio displayed "VaR -32%" when 5th-percentile was actually +32%). Now flips green with "Even the worst 5% of scenarios end above starting value" when applicable; flips red with the loss number when the worst-5% is genuinely negative.
- **Monte Carlo insight text: "the portfolio could decline by X%" rewrites to "even the worst 5% still gained X%"** when the percentile is positive. The fallback insight, the AI-generated insight prompt, and the inline summary all carry the same honest framing.
- **Correlation Heatmap + Drawdown Chart on Simulations tab no longer double-render their own card chrome.** Root cause: both components rendered their own `<div>` with background, border, padding, top accent stripe, AND a duplicate title, inside the parent `<Card><TooltipCardHeader/>` wrapper. Plus CorrelationHeatmap referenced `var(--purple)` for its top stripe - a variable that doesn't exist. Stripped both components down to bare content. DrawdownChart keeps the Max Loss pill (now styled as a rounded badge in the top-right) + the chart. CorrelationHeatmap keeps the Plotly heatmap with theme-aware diverging colorscale (red ↔ navy ↔ green in dark, red ↔ cream ↔ green in light). No more card-in-card nesting, no more invisible variables.
- **Chat error handler classifies upstream Anthropic failures.** The streaming `/chat` exception handler used to collapse every upstream failure into "Chat error. Please try again." regardless of what actually broke. Now logs the full traceback to Railway and maps common failures to plain-English messages: rate limit → "Anthropic rate limit hit. Try again in a few seconds.", overloaded / 529 / 503 → "AI provider is overloaded right now. Try again in a moment.", timeout → "Response timed out. Retry?", context length → "Conversation got too long. Start a new chat?", invalid API key → "Server config issue. Logged."

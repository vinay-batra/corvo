...
### Removed
- **"What should I do today?" card**. Sat directly below the Daily Signal and duplicated its job - one a pre-computed daily insight, the other an on-demand action plan. When the signal was dismissed, the slim "Today's signal dismissed" strip plus the WSID prompt right below it read as visual repetition. DailySignal's existing "Talk to Corvo AI about this" CTA already routes to the chat for free-form follow-ups, so the on-demand path is preserved. Deleted: `wsid` entry in `DASH_CARDS` + `DASH_CARD_LABELS`, `wsidOpen` / `wsidLoading` / `wsidResult` / `wsidError` state, `handleWhatShouldIDo` function, the 180-line JSX block, the customizer group entry, and the section-header gate. Backend `/what-should-i-do` route left in place in case it needs to be re-wired elsewhere later.
...
- **Backend deployed via canonical `railway up`** (manual sequence per CLAUDE.md Deployment section). EOD snapshot cron is now running on the live instance.

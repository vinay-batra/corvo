# Changelog

All notable changes to Corvo are documented here.

---

## v0.43 - May 17, 2026 - 17-commit polish bundle

Rolls up everything shipped after v0.42 without a version bump.

### Added
- **Try-without-signup demo flow.** New `lib/demoPortfolios.ts` defines 3 preset portfolios with stable slugs (`bogleheads`, `nvda-growth`, `dividend-income`). `/app?demo=<slug>` auto-loads the preset assets, runs `fetchPortfolio`, and renders a sticky gold demo banner at the top with a sign-up CTA. `?portfolio=<base64>` shared URLs also auto-run analysis now. AiChat caps anonymous users at 5 messages via a `corvo_anon_chat_count` localStorage counter, after which the input area becomes a "Sign up to keep chatting" panel.
- **Trust signals reinstated.** Homepage `SecurityTrustSection` was defined but never rendered (stale "removed" comment). Wired back in between testimonials and final CTA. 4 cards: bank-grade encryption / never sold or shared / no credit card required / cancel anytime. Auth signup page gained a small inline trust strip below inputs with 3 SVG icons + claims.
- **Monte Carlo Run button + horizon-first UX.** MC no longer auto-fires on tab load. Empty state shows a prominent "Run Simulation" CTA inside a dashed gold container. After results, a small "Re-run" button sits next to the horizon selector. Users pick a horizon first without burning a backend call.
- **Scenario table now shows % alongside $.** Change column reads "+$7,946 (+15.9%)" with same sign / color treatment, 1-decimal formatting.
- **Dashboard tour new "Today's Plan" stop** anchored to the WSID action card (added `id="tour-desk-wsid"` to its outer wrapper). Frames it as the dashboard's main action surface. Desktop tour goes 5 -> 6 stops; mobile stays at 4.
- **Founding-member pricing note** on both paid tiers ("$9/month forever" / "$29/month forever") — loss-aversion framing to drive waitlist conversion.
- **Wheel-to-horizontal scroll** on the testimonial carousel. Vertical mouse-wheel motion over the cards translates to `scrollLeft` movement so the cards move left/right instead of the page scrolling past. Non-passive `wheel` listener; only intercepts when `|deltaY| > |deltaX|` so trackpad horizontal swipes pass through normally.

### Changed
- **Pricing tiers renamed Free / Pro / Elite -> Retail / Accredited / Institutional** (SEC investor classifications — cleaner mapping to who each tier serves than generic SaaS rungs). Both paid tiers show "Coming Soon" pill in-card; Accredited keeps the floating gold "MOST POPULAR" badge for Rule-of-3 anchoring. Feature lists expanded: Accredited gained options Greeks, intraday refresh, custom benchmarks, backtesting; Institutional gained API access, concierge TLH execution, custom alerts on any metric, Fama-French factor decomposition, quarterly 1:1 strategy call.
- **Hero subtext cut 22 words -> 7** ("Retail stays free. Two paid tiers coming soon.").
- **PerformanceChart consolidated chart-level controls into a single footer row.** Stripped the chart-internal sub-header (legend + saved-line toggles + %/$ + Custom range) and merged with the orphaned Max drawdown + Why? chip into one footer below the Plot. Custom-date picker moved from above the chart to below the footer so toggling Custom range doesn't push the chart down.
- **GreetingBar sparkline color now matches today's pct, not 7-day trend.** Mismatch was producing green sparklines next to red pct pills (and vice versa) on the Markets cards.
- **Dashboard tour Step 1 + Step 2 + Step 3 (Tabs) rewritten** to reflect v0.36+ tabbed sidebar, per-holding account-type tagging, Edit-with-Corvo NL editor, the 2-line collapsed brief teaser, the new tab order (Dashboard / Positions / Simulations / Stocks / News), and removed the stale "AI insights" reference.
- **Public nav reordered**: Install moved from #2 to #7 (was breaking the funnel). New: Features / Pricing / Changelog / Blog / About / FAQ / Install.
- **Dashboard tabs reordered**: Stocks and Simulations swapped. New: Dashboard / Positions / Simulations / Stocks / News (owned -> projected -> discover -> context).
- **Pricing copy clarified that the free tier is permanent**, not a beta promo. "Free during beta. Pro coming soon." -> "Corvo's core features are free forever." Subsequent rewrite to "Retail stays free. Two paid tiers coming soon." Both pricing-page and public-FAQ Q&As rewritten.
- **3-card hero demo row removed** as a last touch. The floating "Try a quick analysis" widget on the right side of the hero already covered the "try without signing up" intent, and two parallel demo paths diluted each other. The `/app?demo=<slug>` URLs + demo banner + anon chat cap stay wired in `lib/demoPortfolios.ts` for shareable links and outreach campaigns.

### Fixed
- **SharePortfolio modal renders correctly now.** Root cause: the topbar uses `backdrop-filter: blur(14px) saturate(140%)` which per CSS spec creates a new containing block for `position: fixed` descendants. The share modal's `inset: 0` was being contained by the 56px topbar, producing a thin sliver render with no dark overlay. Fixed by rendering the modal via `createPortal(document.body)` to escape the containing block. Required two attempts — portal must wrap AnimatePresence, not the other way around (AnimatePresence's child-tracking can't reason about a portal return value).
- **Share button weight matched to Export.** Was missing `fontWeight: 600` and rendering at default 400 — visible weight mismatch with the adjacent Export button.
- **Share modal scrolls when tall.** Overlay switched to `alignItems: flex-start + overflowY: auto` so tall content (the canvas preview + buttons + URL input) scrolls within the overlay instead of being pushed off the viewport top.

---

## v0.42 - May 17, 2026 - /chat system prompt expansion

Pure prompt-engineering. No code paths changed, no schema migration, no frontend touch. Layered four new blocks into the existing `/chat` system prompt to make Haiku land in the Corvo voice more reliably and cap length variance across responses.

- **14 example exchanges** as tone + length anchors. Each is a User question paired with a 150-200 word Corvo-voice response. Covers concentration framing, "what should I buy" refusals, missing-data acknowledgements, panic-sell, single-stock drawdown math, cash deployment, extreme weights, lucky-vs-skilled CAGR, jargon definition with the user's actual numbers, web-search-unavailable fallback, multi-portfolio redirect, crash-prediction refusal, rebalance trigger, and tax-loss harvesting in taxable accounts.
- **6 edge-case handlers** prescribing the exact response shape for: empty portfolio, single holding, 100% cash, negative one-year return, Sharpe under 0.5 with positive return, aggressive allocation past 60.
- **Tone calibration anchors**: explicit "too cautious" / "too aggressive" / "too formal" failure modes paired with the matching Corvo-voice rewrite. Direct contrast instead of abstract rule.
- **Roast mode length spec**: 160-190 word target with a 200 hard ceiling, plus a worked 180-word example roast as the length anchor. The previous prompt left roast length unbounded and outputs were drifting past 250 words.

---

## v0.41 - May 17, 2026 - per-holding account types + Monte Carlo realism + chart fixes + chat loosen

### Added
- **Per-holding account-type tagging.** PortfolioBuilder chevron-expand now carries an Account Type selector per holding (cash + non-cash rows). A single saved portfolio can mix Taxable, Roth IRA, Traditional IRA, 401(k), HSA, 529, and Custodial holdings. New `accountType?: AccountTypeId` field on the `Asset` interface; new `HoldingAccountTypeSelector` helper rendered as a native `<select>` grouped by tax category with "Use portfolio default" as the always-present fallback. Persisted via the new `portfolios.holding_account_types text[]` column (migration `20260517000000`) - parallel to `tickers` and `weights`, empty string means "use portfolio default". `SavedPortfolios.fromDb` / `toDb` round-trip the array so reloads survive.
- **Bucketed AI tax advice.** `/chat` reads `portfolio_context.holding_account_types` and, when at least one tag differs from the portfolio default, injects a `PER-HOLDING ACCOUNT BUCKETS` block grouping holdings by bucket and appending each bucket's rules (no TLH in Roth/401k/HSA/529, cap gains only in Taxable, etc). New `_build_account_type_text` helper. Falls back cleanly to v0.32 portfolio-level behavior when no tags are set.
- **Monte Carlo fan chart.** `/montecarlo` returns 250 stratified sample paths as `paths_sample`. MonteCarloChart draws them as semi-transparent hairlines behind the percentile bands so the user sees actual variance, including loss scenarios, instead of an abstract median + band. New "Sample paths" entry in the chart legend.
- **Loss probability surface.** New backend `loss_probability` field (fraction of paths ending below starting value). Frontend shows it inline in the insight: "X% chance of positive returns (Y% chance of a loss)".
- **Chat allows Corvo company / founder / how-it-works questions.** v0.40 system prompt was too tight - "who is the founder of Corvo" got refused as "outside what I can help with here." New `WHAT YOU CAN HELP WITH (do NOT refuse these)` block whitelists portfolio Q&A, Corvo-the-product Q&A (founder bio baked in: Vinay Batra, sophomore solo founder), general financial education, and small talk. Refusals reserved for genuinely out-of-scope (medical / legal / college essays) and capped at one sentence with a pivot back to the portfolio.

### Changed
- **Monte Carlo math rewritten for realism.** Dropped the artificial 8% equity sigma floor that suppressed variance enough to hide losses, switched from Gaussian to Student-t (df=6) innovations for empirical fat tails, lowered the mu cap 12% → 10%, and removed the 0% mu floor in `/portfolio/retirement-simulation` that was papering over poor historical performance. `ASSET_CLASS_MU` expanded with VTSAX / VFIAX / VLCAX / VFWAX / VXUS so mutual-fund-heavy portfolios get a sane anchor. Smoke test on 5y SPY: `final_p5 ≈ -0.76%`, `loss_probability ≈ 5.3%` - the bear band is now genuinely negative instead of cosmetically positive.
- **Monte Carlo path count: 8,500 → 10,000 everywhere.** Updated across 22 surfaces (backend `/montecarlo` + `/montecarlo/insight` + `/portfolio/retirement-simulation`, `lib/api.ts`, MonteCarloChart, RetirementSimulator, dashboard + demo + learn + changelog + FAQ + 4 blog posts + 3 compare-page testimonials, README, CHANGELOG, CLAUDE.md). CLAUDE.md rule updated: "simulations always run exactly 10,000 paths - never 5,000, 8,500, or any other number."
- **Honest table labels.** "Bear Case" / "Bull Case" → "Worst 5%" / "Best 5%". Edge rows flip green/red based on the actual sign of the change rather than the label.
- **VaR and Expected Shortfall cards no longer lie about positive worst-cases.** Used to always render `-X%` in red even when the underlying percentile was positive (so a SPY-heavy 10y portfolio displayed "VaR -32%" when p5 was actually +32%). Now flips green with "Even the worst 5% of scenarios end above starting value" when applicable.
- **Insight text honest about positive bear cases.** "The portfolio could decline by X%" rewrites to "even the worst 5% still gained X%" when the percentile is positive. Applied to the fallback insight, the `/montecarlo/insight` Claude prompt, and the inline summary.
- **Correlation Heatmap + Drawdown Chart no longer double-render card chrome.** Both components used to render their own `<div>` with background / border / padding / top stripe / duplicate title inside the parent `<Card><TooltipCardHeader/>` wrapper. CorrelationHeatmap also referenced a non-existent `var(--purple)`. Stripped both to bare content: DrawdownChart keeps the Max Loss pill, CorrelationHeatmap keeps the Plotly heatmap with a theme-aware diverging colorscale.
- **Chat error handler classifies upstream failures.** The streaming `/chat` `except Exception` used to collapse every failure into "Chat error. Please try again." Now logs the full traceback to Railway and maps common failures to plain-English messages: rate limit, overloaded (529/503), timeout, context-length, invalid API key.

### Migrations
- `supabase/migrations/20260517000000_portfolios_holding_account_types.sql` adds `holding_account_types text[]` to `portfolios`. Idempotent. **Applied to prod.**

---

## v0.40 - May 17, 2026 - API cost reduction (Haiku 4.5 chat + caching + dedup) + quality verification

Commits: [293aae5](https://github.com/vinay-batra/corvo/commit/293aae5) (API cost base), [d7c79bd](https://github.com/vinay-batra/corvo/commit/d7c79bd) (em-dash + word-boundary regex). User added $14 in Anthropic credits; this work drops per-chat cost roughly 5-15x depending on whether the turn needs live data, and adds server-side response caches on two previously-uncached endpoints. Anti-hallucination rules in the system prompt verified via direct quality testing against the live API.

### Added
- **Anthropic prompt caching on `/chat`.** Split the per-request system prompt into two blocks: a static `_CHAT_STATIC_SYSTEM` constant (identical bytes across every call and every user) sent with `cache_control: {type: "ephemeral", ttl: "1h"}`, and a dynamic block carrying portfolio data + investor profile + page context that bypasses the cache. Conversation history's last entry also gets `cache_control: ephemeral` so multi-turn prefixes hit cache after the first turn. Direct probing surfaced that Haiku 4.5's empirical prompt-cache minimum is ~6000 tokens (not the documented 2048); the static prompt is sized at 6261 tokens to clear it. Sonnet 4.6 docs say 1024 minimum and caches cleanly at that size.
- **Server-side response cache on `/what-should-i-do`.** Was firing a fresh Sonnet call on every click. Now uses an `OrderedDict` keyed by `{date}:{user_id}:{account_type}:{period}:{portfolio_shape}`. Day-level freshness matches the prompt's "recommendations must hold for weeks not days" instruction. Capped at 500 entries with LRU eviction.
- **Server-side response cache on `/portfolio/rebalance`.** Same pattern. Key includes `portfolio_value` bucketed to $1k so trivial value nudges don't bust the cache.
- **Conditional `web_search` tool on `/chat`.** Tool is attached per-turn only when the user's message contains keywords from `_CHAT_SEARCH_KEYWORDS` (time markers, news / earnings / analyst / macro / price-action terms). Pure-analysis turns drop the tool to save the ~$0.01 per-search fee. Matched via pre-compiled `_CHAT_SEARCH_RE` with `(?<![a-z0-9])kw(?![a-z0-9])` word-boundary lookbehind/lookahead, so "sharpe ratio" doesn't false-match "pe ratio" and "feedback" doesn't false-match "fed".
- **Anti-hallucination rules in the `/chat` system prompt.** New `FACT GROUNDING`, `WEB SEARCH USAGE`, and `WHEN UNCERTAIN, BE EXPLICIT` sections forbid invented prices, analyst targets, P/E numbers, news, earnings, M&A, executive changes. Rules also explicitly handle the case where web_search is unavailable: the model is required to say "I don't have live market data this turn, but based on your metrics: ..." rather than fabricating. Quality testing confirmed the model correctly refuses to invent Goldman Sachs price targets, admits when it lacks data, and grounds claims in real numbers.
- **Test suite at `/tmp/test_corvo_chat.py`** for direct prompt-quality verification against the live Anthropic API. Covers keyword detection (15 cases), basic advice, three hallucination probes, ROAST MODE length compliance, search-query gating, and prompt-cache activation. Extracts `_CHAT_STATIC_SYSTEM` directly from `backend/main.py` via regex so the test never drifts from what we ship. Runs in ~$0.30 of API spend. Not part of the deploy artifact; kept locally as a regression check.

### Changed
- **`/chat` model swapped Sonnet 4.6 → Haiku 4.5 (`claude-haiku-4-5-20251001`).** Per-message Anthropic cost on tool-free chats drops from ~$0.03-0.05 (Sonnet + always-on web_search) to ~$0.003-0.012 depending on whether the turn needs live data. Haiku 4.5 handles the prompt structure fine when given concrete fact-grounding rules and example exchanges. Anti-hallucination rules added in the same patch (see Added) compensate for the smaller model being more prone to invention than Sonnet was.
- **`/chat` `max_tokens` cut 1024 → 600.** The 150-200 word hard limit needs ~260 output tokens; 600 leaves 2.3x headroom for verbose responses without paying for output that would be truncated by the prompt rule anyway.
- **`send_morning_briefing_emails` hoists the SPY quote fetch out of the per-user loop.** SPY's daily change is the same number for every user but was being fetched N times. Now fetched once at the start of the fire. Saves N-1 Finnhub quote calls per morning. Also added a per-fire `teaser_cache: dict[str, str]` keyed by `{ticker_set}:{pct_str}:{top_mover}:{market_note}` so users with identical portfolio composition + same daily move share one Haiku call instead of N. Removed `display_name` from the prompt (it already lives in the email heading, not the body) so the cache key is safe to dedup on.
- **Em dash post-processor on the `/chat` stream.** Previous post-processor only stripped ` - ` (hyphen with spaces). Haiku 4.5 produces literal `—` (U+2014) and `–` (U+2013) more often than Sonnet did, so the stream now also replaces those characters with `", "`. Prompt rule strengthened with explicit reference to the U+2014 codepoint.
- **ROAST MODE length cap tightened** (further expanded in v0.42). The 200-word "hard limit" was being treated as soft; roasts were running 230-254 words. Added explicit "stop at 200, cut the thought short rather than spill past the limit. This applies to ROAST MODE too" in the HOW TO RESPOND rules. v0.42 layered a 180-word example roast on top as a length anchor.

### Quality verification (15/15 keyword cases + 6/6 prompt tests pass)
- Anti-hall probe: model refused to invent a Goldman Sachs NVDA price target when search was unavailable: "I don't have analyst commentary or specific price targets in your portfolio data, and I can't pull that without a web search this turn."
- Anti-hall probe: model used web_search when asked about current P/E and grounded the answer in real returned data (`45.97 as of May 15, 2026`) rather than guessing.
- Anti-hall probe: model answered the "should I add bonds?" question from portfolio metrics only and did not invent Fed/CPI/rate numbers (search wasn't triggered).
- Format compliance: zero em dashes, zero asterisks, zero disclaimers across all 6 test responses.
- ROAST MODE: 182 words (under the 200-word hard cap), no mid-sentence truncation.
- Prompt cache: `cache_read=6254` tokens on the second tool-free call (full prefix hit).
- Sample response (basic advice): "Trim it. Your 30.0% NVDA weight is outsized for a moderate-risk wealth-building goal over 37 years. Yes, NVDA delivered 78.4% CAGR over the last year, but a concentration that high means a 20% drawdown in NVDA wipes 6% off your total portfolio in one move..." (154 words, Corvo 3-beat pattern, references investor profile).

### Note on local `backend/.env`
- Local `.env` had a stale `ANTHROPIC_API_KEY` (401-ing). Synced to the live Railway value so local `uvicorn` runs work. Gitignored, not committed.

---

## v0.30 - May 12, 2026 - dashboard polish, day-over-day cron, logo overhaul, math fixes, brand-forward AI

### Added
- **Privacy toggle for the live portfolio value**. Eye / eye-off button next to the live dollar amount in GreetingBar header replaces the value with `$••••••` and hides the day delta. Persisted via `corvo_value_hidden` localStorage. Tabular-num figures keep the layout stable across the toggle so nothing reflows. Mirrored in the sidebar Portfolio Value section (PortfolioBuilder) via a new `corvo:value-hidden-changed` custom event so clicking the eye in either spot masks both displays at once.
- **End-of-day portfolio snapshot cron** (`eod_portfolio_snapshot_loop`). Runs at 4:15 PM ET on weekdays, iterates every row in `portfolios`, and calls the existing `_portfolio_snapshot_inner` upsert. Backed by the v0.29 `portfolio_snapshots` schema fix - no new migration required. New admin endpoint `GET /admin/test-eod-snapshot` lets you manually trigger the cron without waiting for market close. Solves the long-standing "tomorrow it resets to the manual base again" problem: the data layer is now there for real day-over-day tracking. GreetingBar reads the snapshot history via the existing `perfHistory` prop and exposes yesterday's daily change in the live-value tooltip; richer visible carry-forward UI lands next iteration once the cron has accumulated a few weekdays of rows.
- **Hash anchor scroll on the homepage**. The homepage wraps content in a 100vh `containerRef` div, so the browser's native scroll-to-`#id` was scrolling the window (which can't move) instead of the actual scrollable container. New `useEffect` listens to `hashchange` and scrolls `containerRef` to the target element on both initial load and subsequent in-page nav clicks. Fixes "click Features in the nav → URL updates but nothing scrolls" - and works for `/#install`, `/#pricing` etc. too. Hardened to retry up to 20× at 100ms intervals if element / scroller aren't ready yet, with a second attempt at 700ms as a fallback because GsapHero's `ScrollTrigger.refresh()` at +120ms was clobbering the initial scroll position.
- **Collapsed-brief preview**. When the Daily / Evening Brief is collapsed, GreetingBar now shows a 2-line teaser pulled from `market.market` (the "Markets Today" snippet) directly under the greeting. `-webkit-line-clamp: 2` truncates cleanly; clicking the preview expands the full brief. Closes the gap where the collapsed brief was just "Good evening, X" with no content cue.
- **Logo overhaul** (multiple iterations, settled here): master `corvo-logo.png` is transparent gold raven with strokes 6px dilated via PIL `MaxFilter` so the silhouette holds at 30-40 px nav sizes. Browser-chrome icons (`favicon-16x16`, `favicon-32x32`, `favicon.ico`, `apple-touch-icon`, `icon-192`, `icon-512`) sit on a dark navy fill at 98% canvas coverage so the raven actually shows up in Chrome tabs, bookmarks, iOS home screen. Master stays transparent so the 26+ inline `<img src="/corvo-logo.png">` callsites sit cleanly on whatever page background they land on. Dashboard sidebar logo bumped 26 → 34 px at full opacity with the same gold drop-shadow PublicNav uses. AiChat avatar bumped 20 → 28 px with the inner ratio raised to 0.72 and a soft gold glow. Footer.tsx and PublicFooter.tsx gain 64 px / 88 px watermark logos centered at the bottom for brand presence. Pipelines kept in `/tmp/corvo_logo_transparent_thick.py` and `/tmp/corvo_favicon_v2.py`.
- **Admin endpoint `GET /admin/real-stats`**. Returns the raw, unfloored platform counts: `auth.users`, `profiles`, `portfolios`, `portfolio_snapshots`, `chat_usage`, `health_score_cache`, `price_alerts`, `price_targets`. The public `/stats` endpoint clamps `user_count` to a 847 floor for the homepage marketing copy - this endpoint reports the truth. Per-table counts use PostgREST's `Prefer: count=exact + Range: 0-0` trick. Failures are swallowed per-table so a single broken table doesn't 500 the whole response. Header: `X-Admin-Key: $SUPABASE_SERVICE_ROLE_KEY`. First snapshot (2026-05-12): 9 profiles / 4 saved portfolios / 55 chat_usage rows / 1 price alert / 2 price targets.
- **Floating chat button → Corvo logo**. The "AI" wordmark on the primary action button read like a thin wrapper over ChatGPT. Now renders the Corvo logo so the brand carries the action. Theme-inverse fill: light theme keeps the gold gradient bg + renders the gold logo as a flat black silhouette via `filter: brightness(0)`; dark theme inverts to a dark fill with the natural gold logo on top. One source PNG either way - no extra asset. Same swap applied to PublicAIChat (the floating button on every public page: features, install, pricing, about, FAQ, changelog, blog). `aria-label` + `title` updated "AI Chat" → "Ask Corvo".
- **"Edit with AI" → "Edit with Corvo"** in the sidebar AI editor accordion. Same brand-forward swap.
- **Reinvest dividends switch.** The Yes / No pill button read like a status badge rather than a control; users didn't realize it was clickable. Replaced with an iOS-style 36×20 toggle switch with a 16px sliding knob, green-filled track + soft glow when on, neutral bg3 + inset shadow when off. The whole row is now a `<label>` wrapping a hidden checkbox so clicking the text or the switch toggles. Same handler, same state.

### Changed
- **Morning / Evening Brief localStorage key bumped** from `corvo_brief_collapsed` to `corvo_brief_collapsed_v2`. Existing users who had inadvertently kept the brief expanded across sessions get reset to the collapsed default; v1 values are ignored. Same `"0"` semantics on the new key.
- **`/watchlist-data` rate limit bumped 30 → 600 per hour.** GreetingBar polls the endpoint every 60 s for index prices and holdings (2 calls per poll), so the previous 30/hour cap locked the dashboard tiles out after about 15 minutes - the live-value, Markets pills, and holdings chips all stayed stuck on "loading...". 600/hour gives multi-tab headroom and absorbs genuine bursts. CLAUDE.md rule corrected: 429s on this endpoint were a UX bug, not normal log noise.
- **GoalTracker projection CAGR clamped to a 4 - 10% long-horizon band.** Previous version compounded the user's observed CAGR (often a 1Y hot run, e.g. 25%) across the full 20-year horizon, producing fantasy projections ($50 k base → $4.7 M target vs $1 M goal). New `projectionCagr = max(0.04, min(observedCagr, 0.10))` is used for the projected value, the trajectory chart, and the required-CAGR math. Card surfaces both numbers and the assumption explicitly: "Projection assumes 10.0% CAGR (capped from your recent 25.5% - long-horizon estimates use 4-10% so a single hot or cold year doesn't dominate the math)". Stat row trimmed 3 → 2 columns; "Projected" sub reframed from "+$3.7M ahead of goal" to neutral "Meets / $X short of $1M". Green celebration footer dropped.
- **PerformanceChart dollar mode now anchors at TODAY.** Previously the chart treated `portfolioValue` as the *start*-of-period value and grew it forward by cumulative return, so a $50 k input showed $62.5 k at the right edge of the chart (cumulative +25% YTD). User-facing inconsistency with every other place in the app that uses `portfolioValue` as a "current" anchor. New implementation derives the implicit start-of-period value by walking cumulative return backwards from today, then plots everything against it - the last data point lands exactly on `portfolioValue`. Benchmark and saved-portfolio overlay use the same implicit start so the comparison reads as "if you'd put $start into S&P / saved-portfolio on day one".
- **WSID "Get Actions" button now actually clickable.** The pill called `e.stopPropagation()` unconditionally but only acted when `wsidOpen + wsidResult` were both true (refresh state). In the initial state the propagation was blocked without doing anything, so clicking the surrounding banner worked but clicking the button itself was a no-op. Now always fires `handleWhatShouldIDo`, clears the cached result first when refreshing, and accepts keyboard (Enter / Space).
- **WSID response parser pairs headline + explanation.** Claude was emitting each action's headline and its explanation as separate blank-line-separated blocks, so a 3-action plan rendered as 6 numbered bullets where every other one was really a paragraph. New parser detects the shape: a short single-line block (<130 char) followed by a substantially longer block (>1.6× length or >120 char) gets paired as headline + explanation. Multi-line blocks still work for backwards compat. Result: 3 bullets, each with paragraph below.
- **CLAUDE.md gains five authoritative rules** (Railway GitHub integration "GENUINELY BROKEN", em dash exception for `.agents/skills/*` plugin docs, `/watchlist-data` 600/hr rate-limit reality, session-scoped caveat on the live portfolio value pattern, projection assumption surfaced explicitly). Open items for v0.29 cleared.
- **AiChat avatar in message bubbles bumped 20 → 28 px**, inner logo ratio 0.58 → 0.72, soft gold glow on the frame. "Always watching" eyebrow above the empty-state headline dropped (the headline already carries the framing). Empty-state logo container bumped 32×26 → 42×42 with proper square aspect.

### Removed
- **AI Insights card.** Duplicated the WSID action card higher up the dashboard - same "AI observations / things to consider" framing, both on the overview tab. Two copies of the same idea in close vertical proximity. Removed: `insights` entry from `DASH_CARDS` + `DASH_CARD_LABELS` + the Analysis group in the customizer, the array entry in the Intelligence bottom-grid, the `AiInsights` import in `app/app/page.tsx`. `frontend/components/AiInsights.tsx` kept on disk in case it gets re-wired elsewhere later.
- **GreetingBar right-column "Portfolio Today" block.** Big 30px green/red percentage + dollar delta inside the brief grid was the exact same data the always-visible GreetingBar header shows next to the greeting. The 30px figure was also overflowing left into the divider on narrower viewports. Right column now goes straight from MARKETS pills into HOLDINGS marquee.

### Reversed mid-release
- **Logo master swung dark-tile → back to transparent (then dilated → back to thin).** Two iterations on the master raven happened mid-day: first the user requested the gold-on-dark-tile design from the reference image, which turned every inline logo into a small dark badge against light page backgrounds and was rejected. Reverted to transparent. Then a 6px alpha dilation was added to thicken the strokes for nav readability, which read as "chunky" and "the bad logo". Reverted to source thin strokes. Browser-chrome favicons retained the dark fill + slightly heftier presentation - they need it for browser tabs / iOS home screens, and the master is rendered inline large enough (22-42px) for thin strokes to read. All raster logo assets (favicon, apple-touch, icon-{192,512}, corvo-pfp, og-image) regenerated from the single source design after the dust settled so there's no design drift across surfaces.
- **Daily Signal removed from the dashboard; WSID restored.** Earlier in v0.30 the WSID card was deleted in favour of consolidating around DailySignal (collapse default + tooltip). User reversed course mid-day: DailySignal pulled out of the dashboard render entirely, all WSID state / handler / JSX / customizer entry / section-header gate restored. Net dashboard: GreetingBar → WSID action card → metrics. DailySignal component file kept at `frontend/components/DailySignal.tsx` and `/daily-signal` backend route still registered; both inert until re-imported / re-called.

### Fixed
- **AiChat hit the segment error boundary on a 25% chance.** `useEffect` rolled `Math.floor(Math.random() * 4)` for the active suggestion set, but `SUGGESTION_SETS` only has 3 entries. Index 3 returned `undefined` and `undefined.map()` in the empty-state render crashed into `app/error.tsx` ("Corvo hit an unexpected error"). Reload re-rolled and usually picked 0-2, masking the bug. Random now capped to `SUGGESTION_SETS.length`; render site has a `?? SUGGESTION_SETS[0]` fallback in case anyone adds or removes a set in the future.
- **Scroll nav not working on the features section**. See "Hash anchor scroll" above for the underlying cause.
- **Portfolio screenshot import was 401-ing for everyone.** `/parse-portfolio-image` was made JWT-protected in v0.28's IDOR closures but the frontend caller in PortfolioBuilder never got updated to send the `Authorization: Bearer <jwt>` header. Every upload hit the backend, `_verify_jwt_user` raised 401, and the client showed a generic "No holdings found" - looked like the model couldn't read the screenshot. Now sends the Supabase access token; backend errors (auth, rate limit, unsupported MIME type) surface their actual message via `d.detail` / `d.error` instead of falling through to the generic catch-all.
- **Portfolio screenshot import prompt sharpened.** Old prompt was "Extract all stock/ETF/crypto holdings..." - didn't mention mutual funds (VTSAX, FXAIX), money markets (SPAXX, VMFXX), or any of the common Vanguard / Fidelity row formats the user was uploading. New prompt explicitly lists the asset types, shows three example row shapes ("VTSAX – 66.07%", "AAPL 25%", "NVDA $12,500 (15%)"), and bans markdown code fences. `max_tokens` bumped 1000 → 2000 so 15-row portfolios don't get truncated. Parser strips code fences if Claude wraps the array, upper-cases tickers, filters malformed rows, and logs raw model output to Railway when extraction fails so operator can debug. Browser MIME `image/jpg` is normalised → `image/jpeg` (Anthropic only accepts canonical form); HEIC / AVIF / PDF / BMP / TIFF return an explicit "Unsupported image type" instead of letting Anthropic generic-400.
- **Navigating to `/#features` glitched at hero then jumped to bento.** Initial-mount auto-scroll fought GSAP's `ScrollTrigger.refresh()` at +120ms. New behavior: if the URL has a hash on first paint (user clicked Features from `/pricing` etc.), strip the hash via `history.replaceState` and stay at hero. In-page hashchange events (user clicks Features while on `/`) still trigger smooth scroll to the section. So nav-from-elsewhere lands clean at hero; in-page clicks still work.
- **PublicNav scroll hide/show belt-and-suspenders.** Kept the `requestAnimationFrame` poll, added a scroll-event listener feeding the same `update()` function. The rAF catches GSAP-driven programmatic scrolls and overflow-container edge cases; the scroll listener catches fast scrolls that exceed the rAF loop's 8px frame-diff threshold. Whichever fires first, the nav slides. Ref attachment retries briefly if `scrollerRef` isn't populated yet.
- **Customize FAB now hides whenever a full-screen panel is on top of the dashboard.** Settings / Profile / Alerts / Email Preferences / Referral / Help / Goals all render as modal overlays at the `/app` route (`activeTab` stays "overview"), so the FAB was peeking behind / next to those panels even though there was nothing to customize. Gate now combines `activeTab === "overview"` with negations on all eight overlay-state flags.

### Restructured
- **Dashboard Intelligence section** ("Where Corvo would focus") now reads: **Health Score** (left, where you are) + **Retirement** (right, where you're heading, slot vacated by AI Insights) + **vs Benchmark** (full-width below, how you stack up). GoalTracker moved out of its standalone Analysis-region block. Goal Tracker renders bare (no `<Card>` wrapper) because it provides its own card chrome - but its chrome now matches the shared dashboard Card spec exactly (border-radius, padding, base shadow, hover lift, soft 24px trackColor glow on hover). Persistent trackColor border-left stripe (red when behind, green when on track) stays as the card's status identity, equivalent of Health Score's gold CardHeader bar.
- **Changelog page (`/changelog`)** redistributed to 5 chapters × 6 entries each, mapping cleanly to v0.1 → v0.30. Era 3 (Mobile, Auth & Math) absorbs v0.18 from old Era 4; Era 4 (Income, Email & Tools) shifts to v0.19 → v0.24; Era 5 (The Guardian Era) extends through v0.30. Adjacent eras no longer overlap on dates.

### Operations
- **Railway "Deploy on Push" disabled at the dashboard level** (`Settings → Source → Auto deploy is disabled`). Backend pushes will no longer trigger broken auto-deploys regardless of `watchPatterns`. Manual `railway up` is the only path now. `railway.toml` keeps the watchPatterns as belt-and-suspenders if anyone ever re-enables auto-deploy.
- **All previously-leaked backend secrets rotated** (Anthropic, Finnhub, Supabase service role, Resend, VAPID). Old keys in public git history are now invalidated. **A second `SUPABASE_SERVICE_ROLE_KEY` rotation pending** - the key was pasted into a Claude chat session for the `/admin/real-stats` query and should be rotated again before end of day.
- **Backend deployed twice via canonical `railway up`** (manual sequence per CLAUDE.md Deployment section). EOD snapshot cron + new admin endpoint + `/watchlist-data` rate-limit bump all live on the running instance.
- **First `/admin/real-stats` snapshot (2026-05-12)**: 9 profiles, 4 saved portfolios, 55 chat_usage rows (assistant turns), 0 portfolio_snapshots (cron hasn't fired its first market-close yet), 0 health_score_cache rows, 1 price alert, 2 price targets. Homepage `/stats` still displays 847+ users due to the marketing floor.

---

## v0.29 - May 11, 2026

### Added
- **New logo**: gold raven head + rising arrow replaces the C-in-gold-circle. Cropped tight, transparent background. Master at `frontend/public/corvo-logo.png` (717×717). Favicon set generated for 16 / 32 / 48 / 180 / 192 / 512 px plus multi-resolution `favicon.ico`. New OG card (1200×630) with raven left + wordmark + tagline. New 400×400 social profile picture on dark fill.
- **Live portfolio value on the dashboard**: GreetingBar shows `base × (1 + today's %)` plus the dollar and percent delta next to "Good evening, Test." - visible even when the brief is collapsed. Sidebar Portfolio Value input mirrors the live number when unfocused, snaps to base on focus, saves user-edited input as the new base on blur.
- **PublicNav refresh**: 68 px tall, content-aligned inner container (1240 max-width / 56 px horizontal padding), hover bubbles on every link and button, fully-rounded gold Get Started pill, bulletproof scroll-aware hide on scroll down / show on scroll up via requestAnimationFrame polling. Homepage uses the same component now.
- **Dashboard tour fixes**: Step 2 (Daily Brief) was anchored to the wrong target - now anchored to the actual brief card. Step 5 consolidated into a single "Top bar" stop covering alerts, theme toggle, Export, Share, and settings.
- **Railway watchPatterns**: scoped auto-deploy to `backend/**`, `railway.toml`, and `Procfile`. Frontend pushes no longer trigger broken Railway build attempts.

### Changed
- **Evening Brief defaults to collapsed** so the Today's Signal card is the dashboard's focal point. localStorage `corvo_brief_collapsed` semantics flipped: only an explicit `"0"` keeps it expanded across sessions.
- **PublicNav structure**: back to 7 flat links (Features, Install, Pricing, Changelog, Blog, About, FAQ) after a brief Linear-style dropdown experiment.
- **Homepage**: replaced its inline 167-line nav with the shared `<PublicNav scrollerRef={containerRef} />`.

### Fixed
- **Hide-on-scroll-up nav was broken on the homepage** even after multiple attempts with scroll event listeners. Switched to `requestAnimationFrame` polling that reads scroll position once per frame regardless of how the page's GSAP container fires events. Works uniformly across every public page now.
- **portfolio_snapshots schema cache miss in prod** - backend was logging `Could not find the 'date' column of 'portfolio_snapshots' in the schema cache`. Root cause: the legacy migration file (`portfolio_snapshots.sql`, no timestamp prefix) was being silently skipped by `supabase db push`, so newer columns never reached prod. New migration `20260511020000_portfolio_snapshots_schema_fix.sql` adds missing columns idempotently and runs `notify pgrst, 'reload schema'` to drop PostgREST's stale cache.
- **Logo coverage gap** - the old C-in-circle was still embedded in `og-image.png` (the social link preview card) and `corvo-pfp.png` (Twitter avatar). Both regenerated with the new raven.
- **Stale public/ assets** deleted: old SVG logo variants, an orphan JPG, and the Next.js default SVG templates (file, globe, next, vercel, window).

---

## v0.28 - May 11, 2026 - audit-driven security + cleanup pass

### Security
- **Closed 3 IDOR vulnerabilities**: `POST /portfolio/snapshot`, `PATCH /price-targets/{id}`, `DELETE /price-targets/{id}`, and `POST /parse-portfolio-image` now JWT-verify the caller and reject when the token user does not match the request user.
- **Authenticated 4 leak routes**: `GET /referrals`, `GET /chat/usage`, `GET /portfolio/history`, and `POST /unsubscribe` now derive `user_id` from the JWT instead of trusting the request.
- **Stripped raw exception strings** from `/chat` SSE and `/parse-portfolio-image` responses (used to leak internal error text including partial Anthropic context).
- **Rate-limit IP source switched to `X-Forwarded-For`** so Railway's proxy doesn't collapse all clients to a single bucket.
- **LRU caps on 8 in-process caches** (`RATE_LIMITS`, `_image_parse_daily`, `_market_per_ticker_cache`, `_sectors_cache`, `_dividends_cache`, `_options_cache`, `_health_score_cache`, `_daily_signal_cache`) via shared `_cap_dict` helper.
- **Added rate limits** to `/prices`, `/search-ticker`, `/market-summary`, `/market-brief`, `/market-driver`, `/earnings-calendar`, `/earnings/transcript/{ticker}`, and `/portfolio/health-score`.
- **Enforced canonical Monte Carlo path count**: `/montecarlo/insight` and `/retirement-simulation` now force `req.simulations = 8500` regardless of client input. (Later bumped to 10,000 with Student-t fat tails in v0.41.)
- **`backend/.env` untracked from git**. The leaked Anthropic key has been rotated; older history still retains it.

### Supabase migrations
- **`20260511000000_security_hardening.sql`**: added `Users read own health scores` RLS policy on `health_score_cache` (was RLS-on with zero policies, locking it to service role only), dropped the wide `Authenticated users can read all profiles` policy, added tight `Users read own profile`, and created a `get_leaderboard(p_limit)` SECURITY DEFINER RPC that exposes only `id`, `display_name`, and `xp`. Frontend leaderboard query migrated to `supabase.rpc("get_leaderboard")`.
- **`20260511000100_drop_paper_trading.sql`**: drops `paper_portfolio` and `paper_trades` tables. Paper Trade was removed from the product in v0.24; the backend routes and frontend component were finally purged in this release.

### Removed
- **Paper Trade** backend routes (`GET /paper-trading/{user_id}`, `GET /paper-trading/{user_id}/history`, `POST /paper-trading/buy`, `POST /paper-trading/sell`, `POST /paper-trading/reset`) and helpers - about 270 lines.
- **Nine unused frontend components**: `PaperTrading`, `PriceTargetTracker`, `DividendTracker`, `EarningsImpactPreview`, `PeerComparison`, `PortfolioCompareTab`, `PortfolioHeartbeat`, `PortfolioHistory`, `MobileBottomNav`. None were imported anywhere.
- **Dead backend route**: `/docs-check`.
- **Emoji** in push notification title (rule violation).
- **Repo cruft**: ~7,400 stale files from `backend/` - duplicate frontend tree, full Streamlit prototype Python files, ghost mirror directory, virtualenv. `backend/` now contains only `main.py`, `requirements.txt`, `.env`, `.env.example`.

### Changed
- **Frontend hardening**: `lib/api.ts` now throws on missing `NEXT_PUBLIC_API_URL` in production instead of silently falling back to localhost. 33 component files migrated to the centralized `RESOLVED_API_URL` export. `middleware.ts` auth call wrapped in try/catch so transient Supabase outage doesn't 500 the site. New `app/error.tsx` segment-level error boundary. GSAP now dynamic-imported inside the hero effect instead of at module scope.
- **Light / dark mode**: 3 compare pages (Bloomberg, Yahoo Finance, Robinhood) converted from hardcoded hex to CSS variables (39 replacements). New `lib/theme.ts` helper (`cssVar`, `plotlyHoverlabel`, `currentTheme`) for libraries that can't read CSS vars directly. Plotly hover labels in 5 charts now theme-aware. Toggle knobs use new `--toggle-knob` / `--toggle-knob-shadow` variables. SharePortfolio canvas resolves CSS vars at runtime. ExportPDF and demo page palettes routed through CSS variables.
- **Mobile**: 7 wrong-breakpoint media queries (max-width 900, 600, 767) normalized to 768. Mobile chart heights overridden on DrawdownChart / MonteCarloChart / CorrelationHeatmap. Wide tables get a `corvo-responsive-table` class that reduces padding + font on mobile. Small icon buttons get a `corvo-touch-44` class enforcing 44×44 minimum touch targets. ProfileEditor 2-col grids collapse to 1-col at `≤768px`. Customize FAB and InstallBanner reposition cleanly on mobile (no more collision with AI / Feedback buttons). OnboardingTour tooltip clamps to viewport. Mobile tab bar bumped from 40 to 44 px and auto-scrolls the active tab into view.
- **Motion / animation**: 34 broken `motion.div` blocks with the `whileInView + initial={false}` anti-pattern (reveal would fire immediately and skip the scroll trigger) fixed with explicit inverse `initial={{ opacity: 0, y: 30 }}` states. `FadeUp` / `SlideIn` / `Reveal` helpers delegate to the IntersectionObserver-based `ScrollReveal`. CLAUDE.md rule updated.
- **Em / en dash sweep**: 330 em dashes and 42 en dashes removed across 79 project source files plus `AGENTS.md` and `CLAUDE.md`. Zero remaining in project source.

### Fixed
- **Background loops dogpiling on cold boot**: 8 loops now stagger their initial work by 30 to 240 s so the backend doesn't fire 8 simultaneous yfinance bursts and trip Yahoo's anti-abuse on every Railway restart.
- **Bare `except:` clauses** (6 of them) promoted to `except Exception:` so they no longer swallow `KeyboardInterrupt` / `SystemExit`.
- **Daily-signal "tied largest holding" guard**: when multiple holdings share the maximum weight, the AI no longer silently picks the first one - returns a generic "N holdings tied" descriptor.
- **Dashboard auto-load `useEffect`**: gained a cancelled flag and a cleanup return so it stops setting state on unmounted components.
- **17 console statements** wrapped in `process.env.NODE_ENV !== "production"` guards.

---

## v0.20 - April 27, 2026

### Added
- **AI chat overhaul:** web search enabled (`web_search` tool on `claude-sonnet-4-6`), streaming responses, available on every page, context-aware portfolio data injected, confident advisor tone enforced
- **Monte Carlo overhaul:** 1-30 year horizon selector, exactly 8,500 simulation paths, monthly GBM steps, advanced settings panel (contributions, inflation, fees, tax drag), histogram distribution view, confidence intervals - state persists across tab switches and theme changes. (Bumped to 10,000 paths with Student-t fat tails and fan-chart visualization in v0.41.)
- **Rebalance Assistant:** drift table showing current vs. target allocation, AI-generated rebalance plan, goal-aware advice, "Continue in AI chat" button
- **Options Chain Viewer:** calls and puts table with delta (Black-Scholes), IV, volume, open interest, ITM highlighting, per-column plain-English tooltips, Max Pain section
- **Transaction Log:** buy/sell trade logging, cost basis tracking, realized P&L calculation, sortable table
- **Health Score upgrade:** daily caching (`health_score_cache` table), advisor-style headline, specific action items referencing real portfolio tickers
- **"What should I do today?" upgrade:** goal-aware advice, never recommends based on single-day moves, "Continue in AI chat" button
- **Email system rebuilt:** Morning Briefing, Week in Review, Monthly Summary, Price Alerts -- all with opt-in toggles in Settings, email theme (light/dark), clean subject lines, teaser format with CTA back to app
- **Auto-load saved portfolio on mount:** returning users see their portfolio immediately, never an empty state
- **Leaderboard fix:** shows all users, RLS policy corrected, "Corvo User" fallback for unnamed users
- **InfoTooltip expansion:** every financial metric, stat, and column header now has a plain-English explanation
- **Morning briefing expand/collapse:** "Expand/Collapse" button replaces bare chevron
- **Price alert editing:** inline edit form with condition and threshold, save and delete
- **Onboarding header:** Corvo logo links to `/app`, user menu in top right

### Fixed
- **Scrolling fixed globally:** `overscroll-behavior: none` set on `html`, `body`, and all major layout containers; passive scroll listeners; no reanimation on elastic scroll
- **Supabase SSR auth:** `middleware.ts` added at repo root, singleton client enforced via `lib/supabase.ts`, inline `createClient` calls removed

---

## v0.19 - April 26, 2026

### Fixed
- **Critical auth bug:** missing `middleware.ts` caused expired JWTs on all SSR requests, silently logging users out
- **Supabase singleton:** `app/page.tsx` was instantiating a second Supabase client without `cookieOptions`, causing sessions to expire on browser close; all client creation now goes through `lib/supabase.ts`
- **Morning briefing price accuracy:** now uses exact live prices from yfinance, never inferred or approximated
- **Onboarding double-render glitch:** step transitions no longer flash or re-mount components

### Added
- **Portfolio performance pill** in dashboard ticker scroll
- **Onboarding header:** Corvo logo (links to `/app`) and user menu with My Account, Referrals, Go To App, Settings, and Logout

---

## v0.18 - April 25, 2026

### Added
- Briefing collapse shows first-sentence preview instead of fully hiding content
- Ticker chips expanded to show price, dollar change, and percent change
- Referral links generated client-side using `/signup` path with full user ID

### Fixed
- Supabase SSR middleware added; inline client creation removed
- Rules compliance audit: dark mode variables, logic safety, icon cleanup

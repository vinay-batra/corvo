...
const C = { amber: "#c9a84c", cream: "#e8e0cc", cream3: "rgba(232,224,204,0.25)", red: "#e05c5c", border: "rgba(255,255,255,0.06)", navy3: "#111620" };
        ...
        {modal!==null&&(
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=>setModal(null)}
            ...
            <motion.div initial={{opacity:0,scale:0.95,y:12}} animate={{opacity:1,scale:1,y:0}} exit={{opacity:0}}
              ...
              style={{background:"#111620",border:"1px solid rgba(255,255,255,0.08)",borderRadius:16,padding:28,maxWidth:400,width:"100%",position:"relative"}}>
              <button onClick={()=>setModal(null)} style={{position:"absolute",top:14,right:14,background:"rgba(255,255,255,0.06)",border:"none",borderRadius:"50%",width:24,height:24,cursor:"pointer",fontSize:11,color:C.cream3}}>✕</button>
              <p style={{fontSize:8,letterSpacing:2,color:C.amber,textTransform:"uppercase",marginBottom:6}}>About</p>
              <h3 style={{fontSize:18,fontWeight:500,color:C.cream,marginBottom:18}}>{EXPLAINERS[modal].title}</h3>
              {[{label:"Plain English",text:EXPLAINERS[modal].simple},{label:"Example",text:EXPLAINERS[modal].example},{label:"What's good?",text:EXPLAINERS[modal].good}].map(({label,text})=>(
                <div key={label} style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:8,padding:"10px 12px",marginBottom:6}}>
                  <p style={{fontSize:8,letterSpacing:2,color:C.amber,textTransform:"uppercase",marginBottom:4}}>{label}</p>
                  <p style={{fontSize:13,color:"rgba(232,224,204,0.7)",lineHeight:1.65}}>{text}</p>
                </div>
              ))}
            </motion.div>
          </motion.div>

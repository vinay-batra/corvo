...
export default function AppPage() {
  ...
  return (
    <div style={{display:"flex",height:"100vh",background:C.navy2,fontFamily:"Inter,sans-serif",color:C.cream}}>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.3}}`}</style>

      {/* SIDEBAR */}
      <aside style={{width:268,flexShrink:0,borderRight:`1px solid ${C.border}`,display:"flex",flexDirection:"column",background:C.navy,overflow:"hidden"}}>
        {/* Logo */}
        ...
        {/* Sidebar body — scrollable, evenly spaced sections */}
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
        {/* Topbar */}
        <header style={{height:44,flexShrink:0,borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 20px",background:C.navy2}}>
        ...
        {/* Content */}
        <main style={{flex:1,overflowY:"auto",padding:"20px 24px",background:C.navy2}}>
          <AnimatePresence mode="wait">
            {!data&&!loading?(
              ...
              <motion.div key="ai" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} style={{height:"calc(100vh - 44px - 48px)"}}>
                <AiChat portfolioContext={{tickers:assets.map(a=>a.ticker),weights:assets.map(a=>a.weight),data}}/>
              </motion.div>
            ):null}
          </AnimatePresence>
        </main>

...
const ERAS = [
  {
    ...
    highlights: [
      ...
      "Sharpe ratio, alpha, beta, max drawdown surfaced from day one with live T-bill risk-free rate",
      "Monte Carlo simulation engine with percentile bands and distribution histograms",
      ...
      "Dividend tracker, tax-loss harvesting, CSV import, PDF export, and shareable card",
    ],
  ...
  },
  {
    ...
    intro: "Corvo learned to talk back, install as a PWA, and survive in your pocket. PWAs, phones, and the math underneath all got serious upgrades.",
    highlights: [
      "Benchmark comparison vs S&P 500 / NASDAQ / Dow + rebalance assistant with full What-If mode",
      "PWA install, push notifications, morning AI brief, weekly digest email, referral system, command palette (⌘K)",
      ...
      "AI chat connected to live web search with streaming + Monte Carlo rebuilt with 1-30 year horizons + Options Chain viewer + Transaction Log",
    ],
  ...
  },
  {
    ...
    intro: "Corvo grew into a real financial workbench: tax, dividends, options, life-aware goals, and proper email.",
    highlights: [
      ...
      "Life events + financial goals in onboarding so AI advice is personalized to age, salary, retirement timeline",
  ...
  },
  {
    ...
    versions: "v0.25 → v0.34",
    dateRange: "May 11 → May 16, 2026",
    intro: "Corvo found its voice — the AI advisor watching over your portfolio — and hardened the surface beneath it.",
    highlights: [
      "Homepage rewrite around the \"your portfolio's guardian\" positioning + auth page rebuild + AI chat polish + app-wide UI standardization",
      "Security audit: 3 IDOR closures, RLS hardening, light/dark theme pass, 330 em dashes swept, leaked secrets rotated",
      "New gold raven logo across favicons, OG card, social pfp + PublicNav 68px refresh + hash-anchor scroll on homepage",
      "Day-over-day EOD snapshot cron + GoalTracker CAGR clamped to a 4-10% long-horizon band + PerformanceChart anchored to today's value",
      "Recently Viewed stocks + Account Type system across 8 wrappers (Taxable / Roth IRA / Trad IRA / Roth 401k / Trad 401k / HSA / 529 / Custodial)",
      "Day-over-day live value persistence + per-account portfolio switcher + GreetingBar account-type pill + same-delta-bug hotfix",
    ],
    tags: ["Brand", "UI", "Security", "Math", "Accounts"],
  },
  {
    num: "05",
    ...
    highlights: [
      ...
      "GreetingBar right-column redesign: market sparkline cards + vertical scrollable Holdings list + sparkline color matched to today's pct",

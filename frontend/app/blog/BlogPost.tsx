  ...
  return (
    <div>
      ...
      <div className="blog-hero-pad" style={{ padding: "80px 56px 48px", maxWidth: 1200, margin: "0 auto", animation: "fadein 0.7s cubic-bezier(0.16,1,0.3,1) 0.1s both" }}>
        ...
        <h1 style={{ fontFamily: "Space Mono,monospace", fontSize: "clamp(24px,4vw,46px)", fontWeight: 700, color: "var(--text)", letterSpacing: -1.5, lineHeight: 1.12, maxWidth: 800, marginBottom: 20 }}>
          {title}
        </h1>
        <p style={{ fontSize: 17, color: "var(--text3)", fontWeight: 300, maxWidth: 680, lineHeight: 1.7, marginBottom: 28 }}>{description}</p>
        <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 32, height: 32, borderRadius: "50%", background: "rgba(201,168,76,0.12)", border: "1px solid rgba(201,168,76,0.25)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <img src="/corvo-logo.png?v=2" width={16} height={14} alt="Corvo" style={{ opacity: 0.85 }} />
            </div>
            <div>
              <p style={{ fontSize: 13, fontWeight: 500, color: "var(--text)", lineHeight: 1 }}>Corvo Team</p>
              <p style={{ fontSize: 10, color: "var(--text3)", marginTop: 3 }}>corvo.capital</p>
            </div>
          </div>
          <div style={{ width: 1, height: 28, background: "var(--border)" }} />
          <p style={{ fontSize: 12, color: "var(--text3)" }}>{date}</p>
          <p style={{ fontSize: 12, color: "var(--text3)" }}>{readTime}</p>
        </div>
      </div>
          ...
          <div className="blog-cards-grid" style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(related.length, 3)},1fr)`, gap: 20, marginBottom: 80 }}>
            {related.map(post => (
              <Link key={post.slug} href={`/blog/${post.slug}`} className="blog-card" style={{ textDecoration: "none" }}>
                ...
                <h3 style={{ fontFamily: "Space Mono,monospace", fontSize: 15, fontWeight: 700, color: "var(--text)", letterSpacing: -0.5, lineHeight: 1.35, marginBottom: 10 }}>{post.title}</h3>
                <p style={{ fontSize: 13, color: "var(--text3)", lineHeight: 1.6, marginBottom: 16, fontWeight: 300 }}>{post.excerpt}</p>
                <p style={{ fontSize: 10, color: "var(--text3)" }}>{post.readTime}</p>
              </Link>
            ))}
          </div>
        </div>
...
export function CtaBox({ text = "Try this in Corvo free", href = "/app" }: { text?: string; href?: string }) {
  return (
    <div style={{ background: "rgba(201,168,76,0.06)", border: "1px solid rgba(201,168,76,0.18)", borderRadius: 16, padding: "32px 36px", margin: "44px 0", textAlign: "center" as const }}>
      <p style={{ fontFamily: "Space Mono,monospace", fontSize: 18, fontWeight: 700, color: "var(--text)", letterSpacing: -0.5, marginBottom: 10 }}>See this in your own portfolio</p>

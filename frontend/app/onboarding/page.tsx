...
function SelectCard({
  ...
  return (
    ...
    >
      ...
      {desc && (
        <div style={{ fontSize: 11, marginTop: 4, color: selected ? "var(--accent-text)" : "var(--text3)" }}>
          {desc}
        </div>
      )}
    </button>
...
function OnboardingContent() {
  ...
  // Step 2: age + income stacked on one card
  const renderAgeAndIncome = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div>
        <div style={{ fontSize: 11, fontWeight: 600, color: "var(--text2)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 10, fontFamily: "var(--font-mono)" }}>
          Age range
        </div>
        <div className="ob-select-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
          {AGE_RANGES.map(opt => (
            <SelectCard
              key={opt.id}
              label={opt.label}
              selected={answers.age_range === opt.id}
              onClick={() => setAnswers(prev => ({ ...prev, age_range: opt.id }))}
            />
          ))}
        </div>
      </div>
        ...
        <div className="ob-select-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
          {INCOME_RANGES.map(opt => (
            <SelectCard
              key={opt.id}
              label={opt.label}
              selected={answers.income_range === opt.id}
              onClick={() => setAnswers(prev => ({ ...prev, income_range: opt.id }))}
            />
          ))}
        </div>
      </div>
  ...
  // Step 4: risk tolerance + horizon stacked on one card
  const renderRiskAndHorizon = () => (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div>
        <div style={{ fontSize: 11, fontWeight: 600, color: "var(--text2)", letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 10, fontFamily: "var(--font-mono)" }}>
          Risk tolerance
        ...
        <div className="ob-select-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
          {RISK_LEVELS.map(opt => (
            <SelectCard
              ...
              label={opt.label}
              desc={opt.desc}
              selected={answers.risk_tolerance === opt.id}
              onClick={() => setAnswers(prev => ({ ...prev, risk_tolerance: opt.id }))}
            />
          ))}
        </div>
      </div>
        ...
        <div className="ob-select-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 8 }}>
          {HORIZONS.map(opt => (
            <SelectCard
              key={opt.id}
              label={opt.label}
              selected={answers.investment_horizon === opt.id}
              onClick={() => setAnswers(prev => ({ ...prev, investment_horizon: opt.id }))}
            />
          ))}
        </div>
      </div>
  ...
  const renderStepContent = () => {
    switch (step) {
      ...
      case 2: return renderAgeAndIncome();
      case 3: return renderPortfolioBuilder();
      case 4: return renderRiskAndHorizon();
      case 5: return renderSingleSelect("referral_source", REFERRAL_SOURCES);
      case 6: return renderFinancialGoals();
      case 7: return <InstallStep />;
      default: return null;
    }
  };
  ...
  const isSkippable = step === 3 || step === 6 || step === 7;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", flexDirection: "column", fontFamily: "var(--font-body)", color: "var(--text)" }}>
      ...
      <main
        className="ob-main"
        style={{ flex: 1, display: "flex", alignItems: "flex-start", justifyContent: "center", padding: "32px 24px 40px", overflowY: "auto" }}
      >
        <div
        ...
        >
          ...
          <h2 style={{ fontSize: 22, fontWeight: 600, color: "var(--text)", margin: "0 0 24px", lineHeight: 1.3, letterSpacing: "-0.3px" }}>
            {STEP_TITLES[step]}
          </h2>

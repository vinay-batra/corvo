    ...
    <div
      ...
      <div
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 24 }}>
          <img src="/corvo-logo.png?v=2" width={28} height={28} alt="Corvo" />
      ...
      >
        <div style={{ maxWidth: 820, margin: "0 auto", textAlign: "center", background: "transparent" }}>
          <div style={{ display: "flex", justifyContent: "center", marginBottom: 24 }}>
            <img src="/corvo-logo.png?v=2" width={32} height={32} alt="Corvo" style={{ opacity: 0.8 }} />
          </div>
...
type PortDemoStep = "input" | "loading" | "results";

interface PortDemoRow {
  ticker: string;
  weight: string;
}

interface PortDemoResult {
  cagr: number;
  volatility: number;
  sharpe: number;
...
const DEFAULT_DEMO_ROWS: PortDemoRow[] = [
    ...
    <div ref={wrapRef} style={{ position: "relative" }}>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 72px 28px", gap: 6, alignItems: "center" }}>
        <input
          ...
          onChange={e => { const v = e.target.value.toUpperCase(); setTouched(true); onTicker(v); if (v.trim().length > 0) setShowDrop(true); else setShowDrop(false); }}
...
function InteractiveDemoWidget({ onDemoStart }: { onDemoStart?: (active: boolean) => void } = {}) {
  ...
  // Typewriter
  useEffect(() => {
    if (!insight || step !== "results") { setTypedInsight(""); return; }
  ...
  const analyze = async () => {
    ...
    const weightStr = validRows.map(r => r.weight).join(",");
    try {
      const [portRes, sectorRes] = await Promise.allSettled([
        fetch(`${API_URL}/portfolio?tickers=${encodeURIComponent(tickerStr)}&weights=${encodeURIComponent(weightStr)}&period=1y&benchmark=%5EGSPC`),
        fetch(`${API_URL}/portfolio/sectors?tickers=${encodeURIComponent(tickerStr)}&weights=${encodeURIComponent(weightStr)}`),
      ]);
      if (portRes.status === "rejected" || !portRes.value.ok) throw new Error("fetch failed");
      ...
      if (sectorRes.status === "fulfilled" && sectorRes.value.ok) {
        const sd = await sectorRes.value.json();
        sectorData = sd.sectors || {};
      }
      const elapsed = Date.now() - loadStart;
      if (elapsed < 1500) await new Promise(r => setTimeout(r, 1500 - elapsed));
      const portResult: PortDemoResult = {
        cagr: portData.annualized_return ?? 0,
        volatility: portData.portfolio_volatility ?? 0,
        sharpe: portData.sharpe_ratio ?? 0,
        benchmarkReturn: portData.benchmark_return ?? null,
        tickers: portData.tickers ?? [],
      };
    ...
    >
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 4 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 5, flexShrink: 0, marginTop: 2 }}>
          ...
          <span style={{ fontSize: 8, letterSpacing: 1.5, color: "var(--green)", textTransform: "uppercase", fontFamily: "Space Mono,monospace" }}>Live Data</span>
        </div>
      </div>
        ...
        <div>
          ...
          {rows.length < 5 && (
            <button
              ...
              style={{
                ...
                cursor: "pointer", padding: "0 0 12px", display: "flex", alignItems: "center",
          ...
          <button
            ...
            style={{
              ...
              border: "none", letterSpacing: 0.2, transition: "all 0.2s",
              background: validRows.length === 0 ? "var(--bg3)" : "var(--accent)",
              color: validRows.length === 0 ? "var(--text3)" : "var(--bg)",
              cursor: validRows.length === 0 ? "not-allowed" : "pointer",
            }}
            onMouseEnter={e => { if (validRows.length > 0) (e.currentTarget as HTMLButtonElement).style.transform = "translateY(-1px)"; }}
      ...
      {step === "loading" && (
        <div>
          <button disabled style={{
            ...
            borderRadius: 10, fontSize: 13, fontWeight: 700, color: "var(--bg)",
            cursor: "not-allowed", marginBottom: 16, display: "flex",
            alignItems: "center", justifyContent: "center", gap: 8, opacity: 0.85,
          }}>
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ animation: "demospin 0.8s linear infinite" }}>
              <circle cx="7" cy="7" r="5.5" stroke="var(--bg)" strokeWidth="1.5" strokeOpacity="0.3"/>
              <path d="M7 1.5 A5.5 5.5 0 0 1 12.5 7" stroke="var(--bg)" strokeWidth="1.5" strokeLinecap="round"/>
            </svg>
            Analyzing...
          </button>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {[0, 0.1, 0.2, 0.3].map((d, i) => (
              <div key={i} style={{ background: "var(--bg3)", borderRadius: 10, padding: "14px" }}>
                ...
                <div style={{ width: "75%", height: 20, background: "var(--border)", borderRadius: 4, animation: `skeletonPulse 1.4s ease-in-out ${d}s infinite` }} />
              </div>
            ))}
          </div>
        </div>
          ...
          >
            ...
            <Link
              ...
              style={{
                ...
                borderRadius: 8, fontSize: 11, fontWeight: 700, color: "var(--bg)",
                textDecoration: "none", transition: "transform 0.15s", whiteSpace: "nowrap" as const,
              }}
...
export default function Landing() {
  ...
  const jsonLd = {
    ...
    "@graph": [
      {
        "@type": "WebApplication",
        "@id": "https://corvo.capital/#app",
        name: "Corvo",
        url: "https://corvo.capital",
        description: "Free institutional-grade portfolio analytics for retail investors. Sharpe ratio, Monte Carlo simulation, AI portfolio chat, tax loss harvesting, dividend tracking and more.",
        ...
        screenshot: "https://corvo.capital/og-image.png?v=2",
      ...
      },
      {
        ...
        name: "Corvo",
        url: "https://corvo.capital",
        logo: "https://corvo.capital/corvo-logo.png?v=2",
      ...
      {
        "@type": "WebSite",
        "@id": "https://corvo.capital/#website",
        url: "https://corvo.capital",
        name: "Corvo",
        publisher: { "@id": "https://corvo.capital/#org" },
        potentialAction: { "@type": "SearchAction", target: "https://corvo.capital/app?q={search_term_string}", "query-input": "required name=search_term_string" },
      },
      {
        "@type": "FAQPage",
        mainEntity: [
          { "@type": "Question", name: "Is Corvo free to use?", acceptedAnswer: { "@type": "Answer", text: "Yes, Corvo is completely free. No credit card required, no subscription needed. All core features including Monte Carlo simulation, Sharpe ratio analysis, and AI chat are available for free." } },
          { "@type": "Question", name: "What is Corvo?", acceptedAnswer: { "@type": "Answer", text: "Corvo is a free portfolio analytics platform that gives retail investors access to institutional-grade tools including Monte Carlo simulation, Sharpe ratio calculation, AI-powered portfolio analysis, tax loss harvesting, and more." } },
          { "@type": "Question", name: "How does Corvo compare to Bloomberg?", acceptedAnswer: { "@type": "Answer", text: "Corvo offers many of the same portfolio analytics features as Bloomberg Terminal but completely free. While Bloomberg costs $24,000/year, Corvo provides Sharpe ratio, Monte Carlo simulation, AI analysis, and more at no cost." } },
          { "@type": "Question", name: "What is a Sharpe ratio?", acceptedAnswer: { "@type": "Answer", text: "The Sharpe ratio measures risk-adjusted return. It's calculated as (portfolio return minus risk-free rate) divided by portfolio volatility. A higher Sharpe ratio means better return per unit of risk." } },
        ],
      },
    ],
  };
  ...
  return (
    <div ref={containerRef} className="page-fadein" style={{ height: "100vh", overflowY: "auto", overflowX: "hidden", overscrollBehavior: "none", background: "transparent", color: "var(--text)", fontFamily: "Inter,sans-serif" }}>
      ...
      {/* SECURITY / TRUST: reinstated between Voices and FinalCTA. Anchors
          financial-data trust before asking for the email signup. */}

...
export default function ReferralsPage() {
  ...
  const platforms: { name: string; color: string; icon: React.ReactNode; action: () => void }[] = [
    ...
    },
    {
      ...
      icon: (
        <svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor">
          <path d="M12.166.009C9.359-.057 6.719 1.282 5.121 3.604 4.347 4.72 3.98 6.084 3.99 7.477c.009.965-.002 1.948-.195 2.896-.108.527-.27.884-.797 1.003-.272.062-.537.034-.79-.058-.161-.057-.319-.13-.483-.169-.3-.072-.618-.071-.887.073-.463.245-.491.786-.316 1.173.32.707 1.078 1.073 1.782 1.285.192.057.388.102.572.176.439.18.606.411.612.879.008.558-.074 1.12-.179 1.668-.203 1.059-.578 2.063-1.166 2.956-.144.219-.305.43-.501.611-.254.236-.546.42-.87.533-.383.133-.789.17-1.191.183-.432.014-.867-.009-1.279.129-.297.099-.556.29-.705.568-.088.166-.088.359.026.512.227.308.629.386.974.454.601.119 1.22.181 1.782.43.45.198.739.581.753 1.078.014.494-.003.576.414.718.416.142.875.149 1.307.214.529.08.921.338 1.173.81.215.408.359.854.623 1.232.338.479.831.795 1.402.905.555.107 1.135.035 1.677-.107.494-.129.981-.282 1.492-.331.47-.045.936.027 1.391.134.473.112.945.267 1.432.335.449.063.916.041 1.341-.104.654-.218 1.085-.77 1.322-1.397.105-.278.169-.567.284-.84.233-.553.652-.858 1.243-.944.373-.055.751-.091 1.12-.166.305-.062.621-.162.835-.39.141-.149.176-.315.14-.512-.073-.4-.376-.643-.738-.783-.375-.144-.773-.169-1.164-.186-.433-.017-.869-.046-1.295-.155-.598-.154-1.027-.558-1.449-1.002-.596-.629-.978-1.39-1.21-2.21-.167-.592-.258-1.199-.252-1.816.003-.397.096-.729.466-.927.219-.116.459-.174.699-.225.256-.054.517-.093.767-.171.629-.197 1.346-.598 1.612-1.263.152-.381.105-.773-.155-1.079-.22-.259-.545-.366-.869-.329-.132.014-.261.051-.389.089-.232.069-.463.157-.704.174-.411.029-.649-.219-.779-.576-.253-.694-.247-1.437-.257-2.162C19.96 4.088 18.13 1.41 15.427.381 14.385-.006 13.264.036 12.166.009z" />
        </svg>
      ),
      action: () => copyForPlatform("Snapchat"),
    },
    {
      name: "TikTok",
      color: "#e8e0cc",
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", color: "var(--text)", fontFamily: "var(--font-body)" }}>
      ...
      <header style={{ height: 52, borderBottom: "0.5px solid var(--border)", display: "flex", alignItems: "center", padding: "0 24px", gap: 16, background: "var(--bg)", position: "sticky", top: 0, zIndex: 10 }}>
        <Link href="/account" style={{ display: "flex", alignItems: "center", gap: 6, color: "var(--text3)", textDecoration: "none", fontSize: 12, transition: "color 0.15s" }}
            ...
            <Section title="Your Impact">
              <div style={{ display: "flex", gap: 12, paddingBottom: 14 }}>
                <div style={{ flex: 1, background: "var(--bg2)", borderRadius: 10, padding: "12px 14px", border: "0.5px solid var(--border)" }}>
                  <div style={{ fontSize: 22, fontWeight: 700, color: "var(--accent)", fontFamily: "var(--font-mono)" }}>{referralData?.referral_count ?? 0}</div>
                  <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 3 }}>Referrals completed</div>
                </div>
                <div style={{ flex: 1, background: "var(--bg2)", borderRadius: 10, padding: "12px 14px", border: "0.5px solid var(--border)" }}>
                  <div style={{ fontSize: 22, fontWeight: 700, color: "var(--accent)", fontFamily: "var(--font-mono)" }}>+{referralData?.bonus_messages_earned ?? 0}</div>

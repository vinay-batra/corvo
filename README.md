...
Current release: **v0.57** (June 8, 2026) - portfolio values that auto-track the market. Your account value now follows the market day-over-day instead of staying frozen at the number you typed: when you set a value, Corvo captures the per-ticker prices at that moment (the anchor) and the live value becomes the weighted buy-and-hold growth of your holdings since then, so it stays close to your real brokerage between updates. Typing a new value re-anchors instantly. The cross-account Portfolio Total now sums each account's live value and ticks intraday. Also: the Account-tab sidebar dropped its duplicate gain/loss line, the Saved tab's "Analyzed" timestamp updates whenever you re-analyze, and the Positions performance chart lets you click any legend label to show/hide that portfolio's line.

Previous: **v0.56** - performance-history reliability (daily snapshots now persist correctly with value-weighted returns + one year backfilled; cross-account total recomputes on edit); **v0.55** - reliability + public-AI (login Turnstile fix, random-logout fix, reliable per-account values, redesigned Ask Corvo panel); **v0.51-v0.54** - onboarding account-type capture + cross-account net-worth panel, PDF export overhaul, light/dark parity + accessibility.

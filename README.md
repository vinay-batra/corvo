...
Current release: **v0.53** (May 28, 2026) — big session. v0.51: onboarding now captures account type upfront + Saved tab shows Net Worth panel with tax-bucket breakdown when 2+ portfolios have values set. v0.52: PDF export overhauled — multi-page support, diverging performance bars (center zero, gold right / red left), sector breakdown fetched inline, benchmark outperform delta, health score card with context label, account type in header; AI Report backend now draws actual bars via ReportLab Flowable and Claude prompt gains full account-type tax framing. v0.53: PublicAIChat panel shadow made theme-adaptive for light mode, user message text fixed (#0a0e14 → var(--bg)), skip-to-content keyboard link added, aria-labels on all icon-only close buttons, saved portfolio chips fully keyboard-navigable, delete dialog gets role=dialog + aria-modal, public changelog extended to v0.52. Supabase email confirmation turned off to reduce signup friction.

...
Current release: **v0.59** (August 23, 2026) - expense ratio tracking, two long-standing CI fixes, 8 user-reported UI/UX fixes, and two live production bugs found and fixed same-day: analyst ratings silently going N/A across every stock during upstream rate-limit windows (now self-heals via automatic retry), and the AI chat contradicting its own "What should I do today?" recommendations when asked to continue the conversation (now explains and extends them instead). All changes verified live against production.

Previous: **v0.58** - security, reliability, and performance hardening (full multi-domain audit -> remediation, CI pipeline, accessibility pass); **v0.57** - portfolio values that auto-track the market day-over-day from a per-account price anchor, so the displayed value stays close to your real brokerage between updates; **v0.56** - performance-history reliability (value-weighted daily snapshots + one year backfilled); **v0.55** - reliability + public-AI (login Turnstile fix, random-logout fix, redesigned Ask Corvo panel); **v0.51-v0.54** - onboarding account-type capture, cross-account net-worth panel, PDF export overhaul, light/dark parity.

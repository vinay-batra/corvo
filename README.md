...
Current release: **v0.58** (June 9, 2026) - security, reliability, and performance hardening. A full multi-domain audit drove a remediation pass that tightened server-side authorization on every personalized endpoint, hardened database row-level security and function permissions, added input validation and rate limits on public endpoints, locked down secrets handling, and cut a large chart dependency to shrink the dashboard bundle. Also added a CI pipeline (build, tests, secret scanning), a pre-commit secret guard, the first automated unit tests, and a full accessibility pass (focus management, labels, keyboard operability, mobile input sizing). Verified live against production.

Previous: **v0.57** - portfolio values that auto-track the market day-over-day from a per-account price anchor, so the displayed value stays close to your real brokerage between updates; **v0.56** - performance-history reliability (value-weighted daily snapshots + one year backfilled); **v0.55** - reliability + public-AI (login Turnstile fix, random-logout fix, redesigned Ask Corvo panel); **v0.51-v0.54** - onboarding account-type capture, cross-account net-worth panel, PDF export overhaul, light/dark parity.

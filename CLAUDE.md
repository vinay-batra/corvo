...
**Last shipped: v0.32 (May 16, 2026) - Recently Viewed stocks on the Stocks tab + Account Type on portfolios (TLH / cap-gains / RMD-aware AI). Bundled with the unshipped v0.31 cache-bust pass. Frontend + backend deploy pending.**
...
1. **Run the v0.32 migration in Supabase BEFORE pushing main**: `supabase/migrations/20260516000000_portfolios_account_type.sql`. Adds `portfolios.account_type` with a CHECK constraint over 8 values + a PostgREST schema-cache reload. Without it, the first PortfolioBuilder save with the new selector will 400 on Supabase rejecting the unknown column.
2. **Deploy backend before frontend**: backend now reads `account_type` from `/portfolio` query params + ChatRequest portfolio_context + 3 new Pydantic fields (HealthScore, WSID, DailySignal). Pushing the frontend first means production gets a v0.32 frontend talking to a v0.31 backend that ignores account_type - the AI advice would silently revert to non-tax-aware. Use the canonical `railway up` sequence in the Deployment section.
3. **Health-score cache schema migration** (deferred): when `account_type` is set, we currently bypass the Supabase `health_score_cache` row entirely because the persisted rows don't carry the column. Plenty of room to land a follow-up that adds `account_type text default ''` to `health_score_cache`, partitions the unique constraint on `(user_id, date, ticker_hash, account_type)`, and stops bypassing.
4. **Onboarding flow doesn't surface account type yet** (intentional): PortfolioBuilder props are optional so onboarding renders without the selector. If we want to capture account type during onboarding later, just thread the same state through that page.

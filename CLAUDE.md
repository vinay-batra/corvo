...
v0.55 (June 1) closed a reliability batch. Key things the next session should know:
...
- **Per-account portfolio value is now reliable.** If it ever regresses, the cause is almost always a stale read: `SavedPortfolios.loadPortfolio()` must read the row's CURRENT `portfolio_value` from Supabase at click time, and the dashboard mount auto-load + both persist effects must seed/respect `lastPersistedPvRef` (anti-clobber). See CRITICAL RULES in memory.

...
**Last shipped: v0.56 (June 4, 2026) - CRITICAL data fix: portfolio history was NEVER being recorded, plus Portfolio Total live-update. (1) THE BIG ONE: `portfolio_snapshots` had 0 rows for ALL 7 portfolios - server-side performance history has been silently dead since the snapshot writer was written. Found by querying live Supabase with the service key, NOT by reading code (the code looked correct and returned 200). Two stacked bugs in `_portfolio_snapshot_inner`: (a) the "upsert" was PATCH-then-POST, but a PostgREST PATCH matching zero rows returns 204 (success, not 404/error), so the code thought it had updated an existing row and the fallback INSERT never ran - every `/portfolio/snapshot` call (the client auto-snapshot AND the 4:15pm ET EOD cron) returned 200 OK while writing nothing; proven live by triggering `/admin/test-eod-snapshot` which returned `{"written":7}` while the table stayed at 0 rows. (b) Underneath it, a schema mismatch: the real table has a legacy `snapshot_date` column that is NOT NULL with no default (the table predates the date/raw_value/portfolio_value/cumulative_return schema in the writer's docstring), which the current writer never set, so even a real insert would have 400'd. FIX: replaced PATCH-then-POST with a single native PostgREST upsert - `POST /portfolio_snapshots?on_conflict=portfolio_id,date` with `Prefer: resolution=merge-duplicates,return=minimal`, and the payload now sets `snapshot_date = today` alongside `date`. NO migration needed: the unique constraint on (portfolio_id, date) already exists (validated by upserting a throwaway sentinel row against the real table - insert 201 + merge 200, one row, then deleted). Deployed via `railway up`; verified live - the cron now writes 7 rows dated 2026-06-04 (portfolio_value 10000 base, cumulative_return 0 on day one) and `/portfolio/history` will populate from here forward. NO BACKFILL: history starts 2026-06-04; prior weeks will not appear unless we seed snapshots from yfinance. (2) Frontend Portfolio Total: the SavedPortfolios aggregate + tax-bucket breakdown was frozen at mount-time values (the panel is always-mounted and only fetched once on load); it now live-updates because the dashboard dispatches `corvo:portfolio-row-updated` after each successful value/account-type persist and SavedPortfolios patches the matching cached row (commit a6d3aa80). LESSON: verify infra against LIVE data, not code reading - this writer "looked correct," returned 200, and had persisted nothing for the table's entire existence. OPEN: two portfolios snapshotted raw_value=1.00 (likely CASH-heavy / unresolved tickers) - glance later; the "track market value while away" auto-ratchet idea is now unblocked (it was impossible against an empty snapshots table). CRITICAL RULE: never revert the snapshot writer to PATCH-then-POST (a zero-match PATCH returns 204 and silently no-ops); keep the on_conflict upsert and keep writing `snapshot_date` until the dead legacy columns are dropped (do both together).**

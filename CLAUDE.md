...
**Last shipped: v0.33 (May 16, 2026) - sidebar ACCOUNT section + GreetingBar account-type pill so the v0.32 tax-context wiring has a visible feedback loop. Frontend only, no backend or migration.**
...
1. **Tighten the 401(k) prompt blocks**: the v0.32 A/B test showed Roth/Trad 401(k) variants were softer than the other 6 (Claude named the wrapper but didn't lean into the tax framing). Easy win: add "name the wrapper in the rationale at least once" to the rule blocks in `_ACCOUNT_TYPE_RULES` for both 401(k) variants. Maybe 4 lines of prompt-engineering.
2. **Health-score cache schema migration** (carry-over from v0.32): when `account_type` is set, we currently bypass the Supabase `health_score_cache` row entirely because the persisted rows don't carry the column. Land a follow-up that adds `account_type text default ''` to `health_score_cache`, partitions the unique constraint on `(user_id, date, ticker_hash, account_type)`, and stops bypassing.
3. **Per-tab portfolio switcher**: most real users have 2-4 accounts (taxable + Roth + 401k + UTMA for kid). Right now SavedPortfolios load is the only way to swap between them; should become an inline chip-row at the top of the workspace. Required for the multi-account YC pitch story.
4. **Onboarding flow doesn't surface account type yet** (intentional carry-over): PortfolioBuilder props are optional so onboarding renders without the selector. If we want to capture account type during onboarding later, thread the state through that page.

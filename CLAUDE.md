...
**Last shipped: v0.57 (June 8, 2026) - portfolio value now AUTO-TRACKS the market day-over-day (reverses the v0.50/v0.56 "typed value is frozen" model), plus 3 smaller fixes. User reported (with Fidelity screenshots) that Corvo values no longer matched Fidelity even though set equal last week. Diagnosed live (service-key Supabase query, NOT code reading - per [[feedback_verify_rls_live]]): NOT a storage bug. Values were stored correctly (UTMA $26,882.13, Roth $20,277.42, Trading $219.46, updated_at Fri Jun 5 ~11am ET); the display was `storedValue x (1 + todayPct)` with a FROZEN base (the deliberate v0.50 decision), so it never carried market moves forward - the user set values Fri morning, the market sold off Fri afternoon (S&P -2.64%), Corvo's base stayed frozen, so Corvo read HIGHER than Fidelity ($317 Roth / $115 UTMA / $442 total). User chose (via AskUserQuestion) "auto-track day-over-day, as accurate as possible." FIX: the live value is now weighted buy-and-hold growth from a per-account ANCHOR: `liveValue = anchorValue * Σ w_i (priceNow_i / anchorPrice_i)`, renormalised over covered weight, with a >=50% coverage guard else fallback to the raw value (shared math in `frontend/lib/anchorValue.ts`, used by both the dashboard and the Total so they can't diverge). When the user sets a value we capture per-ticker prices at that moment (fresh `/watchlist-data` fetch) and store them: new columns `portfolio_value_date date` + `value_anchor_prices jsonb` (migration `20260608000000_portfolios_value_anchor.sql` - ALREADY APPLIED, the columns existed when I verified; idempotent `add column if not exists`, NO SQL left to run). Anchor persisted to Supabase (saved portfolios) + `localStorage corvo_value_anchor` (active/unsaved). Typing a new value RE-ANCHORS instantly (fixes the v0.50 "typed value ignored" bug - typed value always wins). Live prices are polled in `app/app/page.tsx` itself (not read from GreetingBar) so tracking + capture work on EVERY tab. Display: GreetingBar + PortfolioBuilder take a `valueIsLive`/`anchorActive` flag and show the tracked value as-is (no double-applying todayPct). DEPLOY-SAFE / why nothing broke for existing users: all current rows have NULL anchors, so `anchorActive=false` -> EXACT old behavior until a value is re-entered. **ACTION REQUIRED BY USER: re-enter each account's current value ONCE (matching Fidelity now) to capture a fresh anchor; until then it shows the old frozen number.** The cross-account **Portfolio Total** now sums each account's LIVE value (anchor x growth) and ticks intraday - SavedPortfolios polls `/watchlist-data` for the union of tickers (was summing frozen stored values); the active account broadcasts re-anchors via the `corvo:portfolio-row-updated` event (now carries `anchor_prices`/`anchor_date`). SMALLER FIXES: (a) Account-tab sidebar no longer shows the "+\$X +Y% today · base \$Z" line (duplicated the dashboard + showed a fake gain pre-market); (b) Saved-tab "Analyzed ..." now bumps `updated_at` when a saved portfolio is ANALYZED (not just on value/settings edit) + finer granularity (just now / Xm / Xh); (c) Positions performance chart legend labels are now click-to-toggle to show/hide each portfolio line + benchmark (matches the compare-stocks tab; native Plotly legend was disabled there). Commits: `59b108bb` (legend toggle), `b395ab90` (sidebar + timestamp), `f08adec3` (anchor tracking + live Total). Build green; growth math unit-checked (no-move=base, +1% scales, coverage guard + no-anchor fall back). **NOT YET LIVE-VERIFIED in the real UI** (the user re-syncing values is the immediate real test; safe-by-fallback meant pushing was low-risk). CRITICAL RULES: (a) the live value is ANCHOR-BASED - never revert to the static `liveBaseValue = portfolioInputValue` / `value x (1+todayPct)` model (that's what drifted from the brokerage); (b) keep the >=50% coverage guard + raw-value fallback so a partial price fetch never shows a garbage value; (c) typing a value must always re-anchor (typed value wins - do not let any snapshot/derived number override it, the v0.50 bug); (d) `lib/anchorValue.ts` is the single source of the growth math - both call sites must use it. STILL OPEN (task carried over): the performance CHART's daily snapshots are still written at inconsistent intraday times via `yf.download` last-row (Fri's true 4:15pm close was missed; `eod_portfolio_snapshot_loop` apparently didn't overwrite the 11am client snapshot) - the VALUE no longer depends on snapshots, but the chart's recent history is slightly off until the EOD cron is hardened to capture the authoritative close + self-heal missing trading days.**

**Last shipped before: v0.56 (June 4, 2026) - CRITICAL: portfolio history was NEVER recorded AND was computed wrong; fixed both + backfilled 1yr; plus Portfolio Total live-update. Started as "verify snapshots track," uncovered THREE stacked issues in the performance-history pipeline. (1) `portfolio_snapshots` had 0 rows for ALL 7 portfolios - server-side history silently dead since the writer was written. Found by querying live Supabase with the service key, NOT by reading code (code looked correct, returned 200). Bug 1a: the "upsert" was PATCH-then-POST, but a PostgREST PATCH matching zero rows returns 204 (success), so the INSERT fallback never ran - every `/portfolio/snapshot` (client auto-snapshot AND 4:15pm ET cron) returned 200 while writing nothing; proven live (`/admin/test-eod-snapshot` returned `{"written":7}` with the table at 0 rows). Bug 1b: schema mismatch - the real table has a legacy `snapshot_date` NOT NULL (no default) the writer never set, so a real insert would 400. (2) Bug 1c, the deeper one: the cron's `cumulative_return` was WRONG - it divided a price-weighted index (`sum(price*weight)`) by the first snapshot's index, which is NOT a portfolio return (a $400 stock swamped a $1 holding regardless of weight). `/portfolio/calc-history` has the CORRECT method (each ticker normalised to its own base, then weight-summed) but the frontend NEVER calls it (it is dead/unwired - the chart reads ONLY `/portfolio/history`). FIX (all in `_portfolio_snapshot_inner`): single native upsert `POST ?on_conflict=portfolio_id,date` + `Prefer: resolution=merge-duplicates`; set `snapshot_date=today`; rewrote returns to VALUE-WEIGHTED with a FIXED base = snapshot #1's per-ticker prices stored in the `holdings` jsonb (read back each day so the series is stable and matches the backfill); added a coverage guard (skip writing if <50% of base weight resolves, so a flaky partial yfinance fetch can't write a garbage point - this is what caused the earlier raw_value=1.00 rows). NO migration needed (the (portfolio_id,date) unique index already exists). Deployed via `railway up`, verified live. BACKFILLED ~1yr (251 trading days) of correct history for all 7 portfolios via a local script (`/tmp/corvo_backfill.py`, same value-weighted method) - each portfolio's computed return matched the live calc-history endpoint EXACTLY for 6/7 (the 7th, TRIAL 1, exposed calc-history itself returning a bogus -95% vs our robust +24.75% - per-day renorm + guard). Verified consistency: the deployed cron's appended 2026-06-04 point equals the backfill's 2026-06-03 (same base, pre-market prices) - 1762 rows total. NOTE: 2 mutual-fund/money-market-heavy portfolios (TRIAL 1, Total Portfolio) correctly held off today's point (guard: yfinance can't price their funds pre-market); tonight's 4:15pm ET post-close run, when NAVs are set, appends them. (3) Frontend Portfolio Total: SavedPortfolios aggregate + tax buckets were frozen at mount-time (always-mounted panel, fetched once); now live-updates via a `corvo:portfolio-row-updated` event the dashboard dispatches after each value/account-type persist (commit a6d3aa80). DECIDED AGAINST the "track market value while away" auto-ratchet (now unblocked) - it reverses the deliberate v0.50 "typed value is the single source of truth" decision; left as-is. LESSON: verify data infra against LIVE state, not code reading - this pipeline looked correct, returned 200, and had been writing nothing (then would have written WRONG numbers). CRITICAL RULES: (a) never revert the snapshot writer to PATCH-then-POST (zero-match PATCH returns 204 = silent no-op); (b) keep returns VALUE-WEIGHTED off the `holdings` base, never the price-weighted index; (c) the chart depends solely on `/portfolio/history` snapshots (calc-history is unwired); (d) keep writing `snapshot_date` until the dead legacy cols (snapshot_date/total_value/daily_return/sharpe) are dropped (do together). VERIFIED the Portfolio Total fix live in production via a session-injected Playwright test against corvo.capital (admin generate_link -> verify to mint a session, built the @supabase/ssr base64url cookie; editing a saved value moved the cross-account total $50,000 -> $75,132 with no reload; test user/portfolios/snapshots cleaned up afterward, real data intact at 7 portfolios / 1762 snapshots).**

**Last shipped before: v0.55 (June 1, 2026) - reliability + public-AI session. TWO production bugs fixed that mattered most. (1) LOGIN was broken: the CSP never allowed `challenges.cloudflare.com`, so the Turnstile captcha couldn't load, the Log in button (gated on `!captchaToken`) stayed disabled, and login "did nothing." Added challenges.cloudflare.com to script-src + frame-src + connect-src in next.config.ts. (2) RANDOM LOGOUTS: middleware.ts was force-setting `httpOnly:true` on every Supabase cookie (the v0.47 "hardening"), but @supabase/ssr's browser client reads the auth cookie via document.cookie - httpOnly made it unreadable, so the client intermittently saw no session and logged users out. Changed to httpOnly:false (the intended config; kept sameSite=lax + secure). Existing users need ONE clean re-login to mint a fresh readable cookie. PER-ACCOUNT PORTFOLIO VALUE finally reliable: mount auto-load now applies the loaded account's portfolio_value/account_type/reinvest + seeds the persist refs; both persist effects are anti-clobber (adopt baseline on switch, never write the previous account's value); SavedPortfolios.loadPortfolio() reads the CURRENT row value from Supabase at click time (mount cache was stale); onLoad flushes the outgoing account's pending edit. Same-day value: corvo_pv_set_date skips the day-you-set-it market adjustment; decimals supported (step="any"). PUBLIC AI: /api/ai-chat capped 5/IP/day (signed-in unlimited), backend /chat unauthed → chat_anon_daily 5/IP/day, FAQ sends auth token; public chat panel redesigned Lark-style (ASK CORVO eyebrow + X/5 counter + close, centered empty state with icon/title/subtitle/3 suggestion chips, paper-plane send). Polish: "Net Worth"→"Portfolio Total", brief pill shows server ts ("As of X ET · Updates every 5 min"), morning brief parallel/5-min-cache/^GSPC indices/present-tense, dark-mode brief skeleton (--skeleton var), AI report toast createPortal fix, FAQ no page-scroll on reply, Saved tab instant (always-mounted), UserMenu closes on 2nd click + Referrals item removed, About page reframed commercial. Supabase: Roth→roth_ira, UTMA→custodial. Deploy: frontend Vercel git push, backend Railway. LESSON: always `npm run build` before pushing (a nested-IIFE JSX parse error failed a build and looked like "login broken"); prefer git revert over reset+force-push.**

**Last shipped before: v0.54 (May 29, 2026) - morning brief fixes + Portfolio Total label + audit/logic fixes. Backend: parallel yfinance fetches (ThreadPoolExecutor for both index and holdings data), cache TTL 60s->5min, switched index source from ETF proxies (SPY/QQQ/DIA) to actual indices (^GSPC/^IXIC/^DJI) so brief text matches live cards, market-open detection before AI call so brief uses present vs past tense correctly during market hours, double-checked locking on _market_base_cache to prevent concurrent-miss race. Frontend: "Net Worth" panel renamed "Portfolio Total" (tracks Corvo portfolios only, not all real-world assets), "Live now" pill now shows the brief's server-generation timestamp (market.ts) instead of client render time - accurate even with 5-min cache, MarketSummary interface gains ts?: number. Supabase: Vinay Roth fixed taxable_brokerage->roth_ira; Vinay UTMA fixed taxable_brokerage->custodial. Deploy: backend via Railway + frontend via Vercel git push.**

**Last shipped before: v0.53 (May 28, 2026) - light/dark parity + accessibility + changelog update. Light/dark: PublicAIChat panel shadow is now theme-adaptive (soft in light mode, heavy in dark); user message text changed from hardcoded #0a0e14 to var(--bg) so it stays legible in light mode. Accessibility: skip-to-content link in layout.tsx (CSS-only .skip-link class, shows fixed top-left on keyboard focus); aria-label="Close" on InfoModal and FeedbackButton close buttons; SavedPortfolios portfolio chips get role="button" + tabIndex={0} + onKeyDown (Enter/Space) + aria-label; delete confirm dialog gets role="dialog" + aria-modal="true" + aria-labelledby; dashboard main gets id="main-content". Changelog: Chapter 06 extended to v0.42->v0.52, bullet 3 updated for onboarding account type + net worth panel (v0.51), bullet 5 updated for rate-limit fix, bullet 6 replaced with PDF export overhaul (v0.52). Frontend-only, Vercel auto-deploying. Commit: 38128ae.**
...
Key things the next session should know:
- **Portfolio value now AUTO-TRACKS (v0.57) - this is the live value model.** The displayed value is anchor-based: `anchorValue * Σ w_i (priceNow_i/anchorPrice_i)` (shared math `frontend/lib/anchorValue.ts`). Per-account anchor stored in `portfolios.value_anchor_prices` (jsonb) + `portfolio_value_date`. Typing a value re-anchors. Do NOT revert to the static `value x (1+todayPct)` model. NOT yet live-verified in the UI - **first thing to confirm: did the user re-enter values, and does the live value track + stay close to Fidelity?** Legacy rows with NULL anchors fall back to the raw value (no tracking until re-entered once).
- **The performance CHART snapshots are still unreliable (carried-over task).** `_portfolio_snapshot_inner` / `eod_portfolio_snapshot_loop` write at inconsistent intraday times (yf.download last-row); Fri Jun 5's true close was never captured. The live VALUE no longer depends on these, but the chart does. Harden the 4:15pm ET cron to capture the authoritative close + self-heal missing trading days.
- **Login + logout bugs are FIXED but verify after deploy.** Login needed `challenges.cloudflare.com` in the CSP (Turnstile); random logouts needed `httpOnly:false` on Supabase auth cookies. Existing users must do ONE clean re-login to mint a fresh readable 30-day cookie.
- **Per-account value plumbing (still relevant under the anchor model):** `SavedPortfolios.loadPortfolio()` reads the row's CURRENT values from Supabase at click time (now incl. `value_anchor_prices`/`portfolio_value_date`), and the dashboard mount auto-load + persist effects seed/respect `lastPersistedPvRef`/`anchoredForValueRef` (anti-clobber + anti-re-anchor-on-load).
- **Downloads is directly accessible now** (Full Disk Access granted) — edit files normally, no GitHub-API workaround needed.
- **Always `npm run build` in `frontend/` before pushing.** A JSX parse error failed a Vercel build mid-session and looked like "login broken."
- The "redo changelog 6×6 / remove subscribe" request was meant for Lark and was reverted on Corvo — do not re-apply to Corvo.

### Next session - pick up here

1. **Confirm v0.57 value auto-tracking works live** - did the user re-enter their values? Does the displayed value track the market + stay close to Fidelity? Does the Portfolio Total tick intraday? (Build + math verified; UI not yet verified.)
2. **Harden EOD snapshot reliability (performance chart)** - the 4:15pm ET cron must capture the authoritative daily close (not a mid-day yf.download last-row) + self-heal any missing recent trading day. The live value no longer depends on snapshots, but the chart still does.
3. **Demo video** - record the product walkthrough (needs Vinay)
4. **YC application** - Founder Profile Education/Work history + accomplishments still empty (needs Vinay)
5. **Crisis Simulator / Portfolio Time Machine** - new Simulations subtab showing how the user's current portfolio would have performed through 2008, COVID, 2022. Fetch actual historical price data for each ticker, compute portfolio-weighted drawdown, animated chart, Claude narrative. Strong YC demo material.
6. **Accessibility pass round 2** - PortfolioBuilder inputs still lack labels (weight input + ticker search), heading hierarchy on homepage (h3s before h1). Low priority.
...
- Version: v0.55 (frontend + backend both live; login/Turnstile-CSP fix + random-logout/httpOnly-cookie fix + reliable per-account portfolio values + public-AI rate limits + Lark-style public chat redesign shipped June 1)

## What Was Built

### v0.55 (June 1, 2026) - reliability + public AI

**Login fix (CSP / Turnstile).** The auth page gates the Log in button on a Cloudflare Turnstile captcha token (`disabled={loading || !captchaToken}`). The CSP in `next.config.ts` only allowed `vercel.live` in `script-src` / `frame-src`, so `challenges.cloudflare.com` (Turnstile's api.js + challenge iframe) was blocked, the widget never mounted, the token stayed null, and login "did nothing." Added `https://challenges.cloudflare.com` to `script-src`, `frame-src`, and `connect-src`. DO NOT remove.

**Random-logout fix (Supabase cookies).** `middleware.ts` was force-setting `httpOnly:true` on every Supabase cookie (the v0.47 "hardening"). But `@supabase/ssr`'s browser client (`lib/supabase.ts createBrowserClient`) restores the session by reading the auth cookie from `document.cookie` on every page load; httpOnly made it unreadable, so the client intermittently saw no session and logged users out (worse on token refresh). Changed to `httpOnly:false` (the intended config for Supabase SSR; kept `sameSite:"lax"` + `secure` in prod, maxAge flows through for the 30-day session). Existing users need ONE clean re-login to mint a fresh readable cookie.

**Per-account portfolio value made reliable.** Two rounds. (1) Mount auto-load now `select`s + applies the loaded account's `portfolio_value` / `account_type` / `reinvest_dividends` and seeds `lastPersistedPvRef` + `lastPersistedSettingsRef`; both persist effects became anti-clobber (on an account switch where the ref's portfolioId doesn't match, they adopt the current value as baseline and return WITHOUT writing, so they never stamp the previous account's value onto the new one). (2) `SavedPortfolios.loadPortfolio()` reads the row's CURRENT value from Supabase at click time (the mount-cached list went stale on edits - that was the "value resets on switch-back" bug); the dashboard `onLoad` flushes the outgoing account's pending (debounced) edit before switching. Same-day behavior: entering a value writes `corvo_pv_set_date` (ET) and the live display uses multiplier 1 that calendar day (todayPct applies the next trading day - no double-counting). Decimals supported (input `step="any"`, blurred display rounds to cents, header `maximumFractionDigits:2`).

**Public AI: rate limits + Lark-style redesign.** `/api/ai-chat` (the floating public chat) capped at 5 messages/IP/day via an in-memory ET-day store; signed-in users are unlimited (Supabase session detected via `createServerClient` + `cookies()`). Backend `/chat` unauthenticated path switched to a `chat_anon_daily` 5/IP/day key (86400s); the FAQ chat sends the auth token so signed-in users use their per-user daily limit. The `PublicAIChat` panel was redesigned to match the "Ask Lark" layout in Corvo gold: "ASK CORVO" eyebrow + "X / 5 today" counter + close button, centered empty state (trending-up icon badge, title, "Investing questions answered instantly." subtitle, 3 tappable suggestion chips), and a paper-plane send button. Client-side usage counter in `corvo_pub_chat_usage`.

**Morning brief + polish.** `/market-summary`: cache 60s->300s, holdings + index fetches parallelized (`ThreadPoolExecutor`), index source SPY/QQQ/DIA -> ^GSPC/^IXIC/^DJI so the brief's numbers match the live cards, `is_open_now` added to the prompt for present-vs-past tense, double-checked locking on `_market_base_cache`. Brief pill: "Live now" -> "As of {server ts} ET · Updates every 5 min". "Net Worth" panel renamed "Portfolio Total". Dark-mode brief skeleton now visible (new `--skeleton` CSS var; `--bg3` was ~= `--card-bg` in dark). AI report toast rendered via `createPortal` (a Framer transform was breaking `position:fixed`). FAQ chat no longer scrolls the page on reply. Saved tab is instant (component always mounted, display toggle). UserMenu dropdown closes on a second click (per-instance ref instead of `getElementById`, which broke with duplicate desktop/mobile IDs) and the redundant Referrals item was removed (it's in Settings). About page reframed from "personal project" to a commercial product for self-directed retail investors.

**Supabase data:** `Vinay Roth` -> `roth_ira`, `Vinay UTMA` -> `custodial` (run via SQL editor).

**Process lessons:** always `npm run build` in `frontend/` before pushing (a nested-IIFE JSX parse error failed a Vercel build and presented as "login broken"); prefer `git revert` over `git reset --hard` + force-push for undos. A "redo changelog 6x6 / remove subscribe section" request was meant for Lark, applied to Corvo by mistake, then reverted - the Corvo changelog is structurally unchanged.

### v0.54 (May 29, 2026) - morning brief accuracy + Portfolio Total

Backend `/market-summary`: parallel yfinance via `ThreadPoolExecutor`, cache TTL 60s->5min, ETF proxies (SPY/QQQ/DIA) swapped for real indices (^GSPC/^IXIC/^DJI) so the brief text matches the live index cards, market-open detection before the AI call for correct present/past tense, double-checked locking on `_market_base_cache`. Frontend: "Net Worth" -> "Portfolio Total"; the brief "Live now" pill shows the server generation timestamp (`market.ts`) rather than client render time; `MarketSummary` gained `ts?: number`. (Most of v0.54's frontend pill copy was superseded by v0.55's "As of X ET · Updates every 5 min".)
